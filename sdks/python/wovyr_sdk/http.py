...
JSON encode/decode, and error-envelope mapping into `WovyrApiError`.
...
from .errors import WovyrApiError
...
class HttpClient:
    ...
    def _default_headers(self, extra: Optional[Mapping[str, str]] = None) -> dict[str, str]:
        ...
        if self.tenant is not None:
            headers["X-Wovyr-Tenant"] = self.tenant
        if self.principal is not None:
            headers["X-Wovyr-Principal"] = self.principal
        return headers
    ...
    def request(
        ...
        *,
        ...
        """Perform a request and decode the JSON body, raising `WovyrApiError`
    ...
    def request_with_headers(
        ...
        *,
        ...
        if not (200 <= response.status < 300):
            raise WovyrApiError(response.status, _parse_error_body(text), text)
        if not text:
            return None, response.headers
