...
//! [`wovyr_kms`] before it ever reaches disk — closing the gap
...
//! pattern in [`wovyr_secrets::EncryptedFileSecretStore`](../../wovyr-secrets/src/encrypted_store.rs).
//!
...
impl EncryptedFileWebhookStore {
    ...
    fn encrypt(&self, sub: &WebhookSubscription) -> Result<EncryptedSubscription> {
        let sealed_secret =
            wovyr_kms::envelope::seal(self.kms.as_ref(), &sub.tenant, sub.secret.as_bytes())?;
    ...
    fn decrypt(&self, record: &EncryptedSubscription) -> Result<WebhookSubscription> {
        let bytes =
            wovyr_kms::envelope::open(self.kms.as_ref(), &record.tenant, &record.sealed_secret)?;
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "wovyr_webhooks_enc_test_{label}_{}",
            std::process::id()
        ));
