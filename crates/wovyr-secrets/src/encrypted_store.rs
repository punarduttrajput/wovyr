...
//! retained previous value) through [`wovyr_kms`] before it ever reaches
...
impl EncryptedFileSecretStore {
    ...
    fn seal(&self, tenant: &str, plaintext: &str) -> Result<SealedData> {
        wovyr_kms::envelope::seal(self.kms.as_ref(), tenant, plaintext.as_bytes())
    }

    fn unseal(&self, tenant: &str, sealed: &SealedData) -> Result<String> {
        let bytes = wovyr_kms::envelope::open(self.kms.as_ref(), tenant, sealed)?;
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "wovyr_secrets_enc_test_{label}_{}",
            std::process::id()
        ));
