...
fn scratch_dir(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "apex_workflow_crash_recovery_{name}_{}",
        std::process::id()
    ));
...
fn dummy_event() -> apex_workflow::WorkflowEvent {
    apex_workflow::WorkflowEvent::WorkflowStarted
}
...
/// apex-kms and apex-common tests do — must leave the last *committed*
