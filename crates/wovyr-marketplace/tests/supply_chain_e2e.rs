...
//! cover the two halves in isolation — `wovyr-marketplace` proves publish→discover→
//! download, `wovyr-plugin` proves install→enable→execute — but nothing connects them
...
//! feature: `cargo test -p wovyr-marketplace --features wasi`.

use ring::signature::{Ed25519KeyPair, KeyPair};
use std::path::PathBuf;
use wovyr_marketplace::{InMemoryRegistryStore, PermissionRisk, Registry, SearchQuery};
use wovyr_plugin::{CapabilityKind, Package, PluginEngine, TrustStore};
...
fn staging_dir(tag: &str) -> PathBuf {
    std::env::temp_dir().join(format!("wovyr_mkt_e2e_{tag}_{}", std::process::id()))
}

/// Sign the example's manifest with a fresh ed25519 key (standing in for the
/// publisher's signing identity) and build a `.wovyrpkg` carrying the real wasm module.
...
fn signed_example_pkg() -> (Vec<u8>, TrustStore) {
    ...
    let wovyrpkg = Package::new(manifest_yaml, sig)
        .with_artifact("echo.wasm", wasm)
        .to_wovyrpkg()
        .expect("encode .wovyrpkg");
...
fn example_plugin_publishes_discovers_downloads_and_installs() {
    let (wovyrpkg, trust) = signed_example_pkg();
    ...
    let out = registry
        .publish(&wovyrpkg, &["devtools".into()], None)
    ...
    let package = Package::from_wovyrpkg(&downloaded).expect("reparse downloaded .wovyrpkg");
    let mut engine =
    ...
    let (wovyrpkg2, trust2) = signed_example_pkg();
    let package2 = Package::from_wovyrpkg(&wovyrpkg2).unwrap();
...
async fn example_plugin_capability_executes_end_to_end() {
    use serde_json::json;
    use wovyr_plugin::WasiCapabilityRuntime;
    ...
    let package = Package::from_wovyrpkg(&downloaded).expect("reparse");
