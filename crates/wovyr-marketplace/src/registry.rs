...
//!    (reusing [`Package::verify`](wovyr_plugin::Package::verify)), so an untrusted
...
use wovyr_plugin::keyless::{IdentityPolicy, KeylessRoot};
...
impl<S: RegistryStore> Registry<S> {
    ...
    /// Publish a `.wovyrpkg` package to `channel` (default [`DEFAULT_CHANNEL`] if `None`).
    ///
    ...
    pub fn publish(
        ...
        let version = PublishedVersion {
            ...
            scan: scan.clone(),
            package_digest,
            package: String::from_utf8(wovyrpkg.to_vec())
                .map_err(|_| Error::invalid("`.wovyrpkg` is not valid UTF-8 JSON"))?,
        };
    ...
    /// Download the `.wovyrpkg` bytes for a version (the latest stable if `version` is
...
mod tests {
    ...
    /// Build a signed `.wovyrpkg` for `manifest_yaml`, returning (bytes, public_key).
...
"#;
    ...
    fn publish_then_discover_and_download() {
        ...
        let parsed = Package::from_wovyrpkg(&bytes).unwrap();
    ...
    fn multiple_versions_resolve_channel_and_specific_download() {
        ...
        let mk = |yaml: &str| {
            ...
            Package::new(yaml, sig).to_wovyrpkg().unwrap()
        };
        let p1 = mk(GITHUB);
        let p2 = mk(&v2_yaml);
        let _ = (v1, public);
    ...
    fn keyless_publish_accepted_when_configured_rejected_otherwise() {
        ...
        let policy = IdentityPolicy {
            allow: vec![wovyr_plugin::keyless::IdentityRule {
                issuer: "https://ci.example.com".into(),
                subject: "release@acme.dev".into(),
                publisher: "acme".into(),
            }],
        ...
        assert!(
            Package::from_wovyrpkg(&downloaded)
                .unwrap()
                .keyless_bundle()
                .is_some()
        );
