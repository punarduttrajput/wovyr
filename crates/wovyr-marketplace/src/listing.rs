...
use wovyr_plugin::{CapabilityKind, is_broad};
...
pub struct PublishedVersion {
    ...
    /// The published package as `.wovyrpkg` JSON text (the install artifact). Held as a
    /// `String` because the `.wovyrpkg` envelope is itself UTF-8 JSON.
    pub package: String,
}
