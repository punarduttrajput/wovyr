...
//! listing is [`Error::NotFound`](wovyr_common::Error::NotFound).
...
impl FileRegistryStore {
    ...
    fn mutate<T>(&self, f: impl FnOnce(&mut RegistryState) -> Result<T>) -> Result<T> {
        ...
        let _flock = wovyr_common::fs::FileLock::acquire(lock_dir)
...
mod tests {
    ...
    fn file_store_round_trips() {
        let dir = std::env::temp_dir().join(format!("wovyr-mkt-test-{}", std::process::id()));
    ...
    fn file_store_round_trips_abuse_reports() {
        let dir = std::env::temp_dir().join(format!("wovyr-mkt-abuse-test-{}", std::process::id()));
        let path = dir.join("registry.json");
        let _ = std::fs::remove_file(&path);
        let store = FileRegistryStore::new(&path);
        store
