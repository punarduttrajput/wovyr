...
//! `wovyr_workflow_schema_history` table distinct from the other Postgres-backed
...
//! ever runs DDL — invoked explicitly via `wovyr admin migrate`, never by
...
/// tracking colliding.
const MIGRATION_TABLE: &str = "wovyr_workflow_schema_history";
        ...
        .map_err(|e| {
            Error::config(format!(
                "workflow Postgres schema is not migrated (expected version {expected}): {e}; \
                 run `wovyr admin migrate --target workflow --database-url <url>` first"
            ))
        })?
        .map(|m| m.version())
        .unwrap_or(0);
    if applied < expected {
        return Err(Error::config(format!(
            "workflow Postgres schema is at version {applied}, but this binary needs version \
             {expected}; run `wovyr admin migrate --target workflow --database-url <url>`"
        )));
    }
    if applied > expected {
        return Err(Error::config(format!(
...
///
/// - TLS is used when the URL requests it (`sslmode=require`) or `WOVYR_PG_TLS=1` is set.
    ...
    if !all_loopback && !wants_tls {
        return Err(Error::config(
            "refusing a plaintext Postgres connection to a non-loopback host; use \
             `sslmode=require` (or set WOVYR_PG_TLS=1) so credentials and data are \
...
/// verification with `WOVYR_PG_TLS_VERIFY=1`.
#[derive(Debug)]
...
/// roots when `WOVYR_PG_TLS_VERIFY=1` (for a public-CA host); otherwise encrypts without
...
fn rustls_connector() -> Result<tokio_postgres_rustls::MakeRustlsConnect> {
    ...
    let config = if std::env::var("WOVYR_PG_TLS_VERIFY")
        .map(|v| v == "1")
        .unwrap_or(false)
    {
...
/// needs here are modest, matching how wovyr hand-rolls its S3 signer / cron evaluator.
struct PgPool {
    conn_str: String,
    tls: TlsMode,
...
impl PostgresStore {
    ...
    pub async fn connect(conn_str: &str) -> Result<Self> {
        ...
        let max_size = std::env::var("WOVYR_PG_POOL_MAX")
            .ok()
    ...
    /// `wovyr admin migrate`, not from `connect`/`serve`, so the serving path
...
mod tests {
    ...
    /// without a database. (Assumes `WOVYR_PG_TLS` is unset, as it is in CI/local test
