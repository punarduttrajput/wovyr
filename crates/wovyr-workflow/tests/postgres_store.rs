...
//! `WOVYR_WORKFLOW_POSTGRES_URL` and skips (logging) when unset/unreachable, so
...
//!
//! ```bash
//! cargo run -p wovyr-cli --features postgres -- admin migrate --target workflow \
//!   --database-url postgres://wovyr:wovyr@127.0.0.1:5433/wovyr
//! WOVYR_WORKFLOW_POSTGRES_URL=postgres://wovyr:wovyr@127.0.0.1:5433/wovyr \
//!   cargo test -p wovyr-workflow --features postgres --test postgres_store -- --nocapture
//! ```
//!
...
async fn store() -> Option<Arc<PostgresStore>> {
    let url = match std::env::var("WOVYR_WORKFLOW_POSTGRES_URL") {
        Ok(u) => u,
        Err(_) => {
            eprintln!("skipping: WOVYR_WORKFLOW_POSTGRES_URL not set");
            return None;
        }
    };
...
async fn distributed_workers_over_postgres_process_each_exactly_once() {
    ...
    let url = std::env::var("WOVYR_WORKFLOW_POSTGRES_URL").unwrap();
...
/// doesn't understand. `wovyr-memory`/`wovyr-marketplace`'s `PostgresStore`
...
async fn connect_refuses_a_schema_newer_than_this_binary_understands() {
    ...
    raw.execute(
        "INSERT INTO wovyr_workflow_schema_history (version, name, applied_on, checksum)
    ...
    raw.execute(
        "DELETE FROM wovyr_workflow_schema_history WHERE version = 999999",
        &[],
    )
    .await
    .unwrap();
...
async fn concurrent_appends_to_one_execution_get_distinct_contiguous_seqs() {
    use wovyr_workflow::WorkflowEvent;
    let Some(store) = store().await else { return };
...
/// `WOVYR_WORKFLOW_POSTGRES_TLS_URL` — with `sslmode=require` already in the
/// URL — so this can run against a genuinely TLS-configured Postgres alongside
/// the plaintext one `WOVYR_WORKFLOW_POSTGRES_URL` points at; CI's
...
async fn connects_over_a_real_tls_handshake_with_sslmode_require() {
    let url = match std::env::var("WOVYR_WORKFLOW_POSTGRES_TLS_URL") {
        Ok(u) => u,
        Err(_) => {
            eprintln!("skipping: WOVYR_WORKFLOW_POSTGRES_TLS_URL not set");
            return;
        }
    };
    assert!(
