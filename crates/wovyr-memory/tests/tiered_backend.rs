...
//! sandbox tests (`wovyr-tools/tests/sandbox_backends.rs`): each test reads
//! `WOVYR_MEMORY_POSTGRES_URL` / `WOVYR_MEMORY_QDRANT_URL` and returns early (logging a
...
//!
//! ```bash
//! cargo run -p wovyr-cli --features tiered-memory -- admin migrate --target memory \
//!   --database-url postgres://wovyr:wovyr@127.0.0.1:5433/wovyr
//! WOVYR_MEMORY_POSTGRES_URL=postgres://wovyr:wovyr@127.0.0.1:5433/wovyr \
//! WOVYR_MEMORY_QDRANT_URL=http://127.0.0.1:6333 \
//!   cargo test -p wovyr-memory --features tiered --test tiered_backend -- --nocapture
//! ```
//!
//! Data is isolated by a per-run **nonce namespace**, so repeated runs are
//! independent and don't collide. A single shared Qdrant collection (`wovyr_it_mem`)
...
const COLLECTION: &str = "wovyr_it_mem";
...
async fn pg() -> Option<PostgresStore> {
    let url = match std::env::var("WOVYR_MEMORY_POSTGRES_URL") {
        Ok(u) => u,
        Err(_) => {
            eprintln!("skipping: WOVYR_MEMORY_POSTGRES_URL not set");
            return None;
        }
    };
...
async fn tiered() -> Option<TieredStore> {
    let (Ok(pg_url), Ok(qd_url)) = (
        ...
        eprintln!("skipping: WOVYR_MEMORY_POSTGRES_URL / WOVYR_MEMORY_QDRANT_URL not both set");
        return None;
    };
...
async fn in_process_bm25_agrees_with_postgres_fts_on_the_top_result() {
    use wovyr_memory::InMemoryStore;
