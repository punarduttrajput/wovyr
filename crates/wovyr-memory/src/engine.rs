...
impl MemoryEngine {
    ...
    /// through `wovyr-kms` before it reaches disk.
...
mod tests {
    ...
    use wovyr_provider::MockProvider;
    ...
    fn try_new_fails_closed_without_an_embedding_provider() {
        use wovyr_provider::AnthropicProvider;
    ...
    #[async_trait::async_trait]
    impl wovyr_provider::AIProvider for AltEmbedProvider {
        fn name(&self) -> &str {
            "mockv2"
        }
        async fn chat(
            ...
            Err(wovyr_common::Error::provider(
                "chat is not used in this test",
            ))
        }
        async fn embed(
            &self,
            request: wovyr_provider::EmbeddingRequest,
        ) -> wovyr_common::Result<wovyr_provider::EmbeddingResponse> {
            Ok(wovyr_provider::EmbeddingResponse {
                model: request.model,
                vectors: request
                    .input
                    .iter()
                    ...
                    .collect(),
                usage: wovyr_common::Usage::default(),
            })
        }
    }
    ...
    async fn migration_fails_closed_on_a_store_without_update() {
        ...
        impl MemoryStore for NoUpdateStore {
            async fn put(&self, record: MemoryRecord) -> wovyr_common::Result<String> {
                self.0.put(record).await
            }
            async fn all(
                &self,
                namespace: Option<&str>,
            ) -> wovyr_common::Result<Vec<MemoryRecord>> {
