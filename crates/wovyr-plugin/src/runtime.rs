...
//! — `{"__wovyr_abi": 1, "params": <params>, "secrets": {"WOVYR_SECRET_<NAME>": …}}` —
...
//! `wovyr-plugin-sdk` reads both shapes transparently. [`SecretChannel::Env`] is the
//! legacy path (`WOVYR_SECRET_*` env vars, bare-params stdin) for native entries
...
pub enum SecretChannel {
    ...
    /// (`{"__wovyr_abi": 1, "params": …, "secrets": …}`) and never enter the
    /// guest's environment. The default.
    #[default]
    Stdin,
    /// Legacy: secrets are injected as `WOVYR_SECRET_*` environment variables and
...
pub struct ContainerCapabilityRuntime {
    ...
    secrets: Option<wovyr_secrets::Vault>,
...
impl ContainerCapabilityRuntime {
    ...
    pub fn with_secrets(mut self, vault: wovyr_secrets::Vault) -> Self {
...
mod secret_channel_tests {
    ...
    fn secrets() -> Vec<(String, String)> {
        vec![("WOVYR_SECRET_API_TOKEN".to_string(), "s3cr3t".to_string())]
    }
    ...
    fn stdin_channel_envelopes_secrets_and_keeps_env_empty() {
        ...
        assert_eq!(doc["__wovyr_abi"], 1);
        assert_eq!(doc["params"], params);
        assert_eq!(doc["secrets"]["WOVYR_SECRET_API_TOKEN"], "s3cr3t");
    }
...
mod tests {
    ...
    use serde_json::json;
    use wovyr_tools::{ToolContext, ToolRegistry};
...
"#,
    ...
    fn vault_with_token() -> wovyr_secrets::Vault {
        let store = std::sync::Arc::new(wovyr_secrets::InMemorySecretStore::new());
        let vault = wovyr_secrets::Vault::new(store);
    ...
    async fn stdin_channel_keeps_wasi_environ_empty_and_delivers_via_envelope() {
        let staging =
            std::env::temp_dir().join(format!("wovyr_plugin_wasi_sec_{}", std::process::id()));
        ...
        assert_eq!(payload, json!(1), "the env channel injects WOVYR_SECRET_*");
        ...
        assert_eq!(payload["__wovyr_abi"], json!(1));
        assert_eq!(payload["params"], json!({"q": "x"}));
        assert_eq!(payload["secrets"]["WOVYR_SECRET_API_TOKEN"], "s3cr3t-t0ken");
    ...
    async fn wasm_capability_round_trips_request_to_response() {
        let staging =
            std::env::temp_dir().join(format!("wovyr_plugin_wasi_{}", std::process::id()));
...
/// `wovyr-tools/tests/sandbox_backends.rs` (probe, log a skip, return early).
#[cfg(test)]
mod container_tests {
    use super::*;
    use crate::engine::{Package, PluginEngine};
    ...
    use crate::verify::testing::{generate_keypair, sign};
    use serde_json::json;
    use wovyr_tools::{SandboxManager, ToolContext, ToolRegistry, ToolRequest};
    ...
    /// including its `WOVYR_SECRET_API_TOKEN` env var.
    const ECHO_SH: &str = "#!/bin/sh\n\
        IN=$(cat)\n\
        printf '{\"echo\":%s,\"token\":\"%s\"}' \"$IN\" \"$WOVYR_SECRET_API_TOKEN\"\n";
...
"#,
    ...
    /// `SecretChannel::Env` — the script reads `$WOVYR_SECRET_API_TOKEN`).
    #[tokio::test]
    async fn container_capability_runs_end_to_end() {
        ...
        let staging =
            std::env::temp_dir().join(format!("wovyr_plugin_container_{}", std::process::id()));
    ...
    /// `$WOVYR_SECRET_API_TOKEN` env var alongside — the latter must be empty.
    const STDIN_SECRET_SH: &str = "#!/bin/sh\n\
        IN=$(cat)\n\
        TOK=$(printf '%s' \"$IN\" | sed -n 's/.*\"WOVYR_SECRET_API_TOKEN\":\"\\([^\"]*\\)\".*/\\1/p')\n\
        printf '{\"token_from_stdin\":\"%s\",\"token_from_env\":\"%s\"}' \"$TOK\" \"$WOVYR_SECRET_API_TOKEN\"\n";
    ...
    async fn stdin_channel_delivers_secret_without_touching_guest_env() {
        ...
        let staging =
            std::env::temp_dir().join(format!("wovyr_plugin_stdin_sec_{}", std::process::id()));
...
"#,
    ...
    async fn gvisor_capability_runs_end_to_end() {
        ...
        let staging =
            std::env::temp_dir().join(format!("wovyr_plugin_gvisor_{}", std::process::id()));
