...
//! [`ToolRegistry`](wovyr_tools::ToolRegistry) — so the engine registers a
...
impl Package {
    ...
    /// Serialize to the single-file **`.wovyrpkg`** distribution format
    /// ([distribution §2](../../docs/08-plugin-sdk/distribution.md#2-package-format-wovyrpkg)):
    ...
    pub fn to_wovyrpkg(&self) -> Result<Vec<u8>> {
        let envelope = WovyrPkg {
    ...
    /// Reconstruct a package from `.wovyrpkg` bytes (the inverse of
    /// [`to_wovyrpkg`](Self::to_wovyrpkg)). The signature and artifacts are re-verified
    /// at install, so a tampered envelope is rejected there.
    pub fn from_wovyrpkg(bytes: &[u8]) -> Result<Self> {
        let envelope: WovyrPkg = serde_json::from_slice(bytes)
            .map_err(|e| Error::invalid(format!("invalid .wovyrpkg: {e}")))?;
...
/// The on-disk `.wovyrpkg` envelope: manifest YAML + hex signature + hex artifact
...
/// durable catalog (e.g. the CLI's `~/.wovyr/plugins/catalog.json`) survives across
/// processes.
...
impl PluginEngine {
    ...
    /// engine from `~/.wovyr/plugins/catalog.json` without re-running install.
...
/// reference, or an absent secret) and exposed to the guest as `WOVYR_SECRET_<NAME>`.
///
...
pub(crate) fn resolve_secret_env(
    ...
    let access = wovyr_secrets::SecretAccess::new(tenant, declared_permissions.to_vec());
    let mut env = Vec::new();
    for perm in declared_permissions {
        ...
        let reference = wovyr_secrets::SecretRef::new(tenant, name)
...
/// Map a secret name to the env var it is injected as: `WOVYR_SECRET_<UPPER_SNAKE>`
...
mod tests {
    ...
    /// `snake_case` — `wovyr-server`'s `GET /api/v1/plugins` response used to
...
"#;
    ...
    #[test]
    fn wovyrpkg_round_trips_and_installs() {
        // Pack a signed package to .wovyrpkg bytes, reconstruct it, and install — the
        // reconstructed package must verify (signature + artifact digest) intact.
        let (package, mut engine) = signed_package();
        let bytes = package.to_wovyrpkg().unwrap();
        let restored = Package::from_wovyrpkg(&bytes).unwrap();
...
"#;
    ...
    fn vault_with(tenant: &str, name: &str, value: &str) -> wovyr_secrets::Vault {
    ...
    fn resolves_declared_secret_into_env() {
        ...
        assert_eq!(
            env,
            vec![(
                "WOVYR_SECRET_VPN_ADMIN_TOKEN".to_string(),
