//! Property/fuzz tests for the manifest YAML and `.wovyrpkg` envelope parsers
...
use wovyr_plugin::{Package, PluginManifest};
...
proptest! {
    ...
    /// Arbitrary bytes fed to the `.wovyrpkg` envelope parser must never panic.
    #[test]
    fn wovyrpkg_from_bytes_never_panics(input in proptest::collection::vec(any::<u8>(), 0..2048)) {
        let _ = Package::from_wovyrpkg(&input);
    }
    ...
    /// A structurally-plausible-but-fuzzed `.wovyrpkg` JSON envelope must not panic.
    #[test]
    fn wovyrpkg_envelope_with_plausible_shape_never_panics(
        manifest in ".{0,200}",
        ...
        let _ = Package::from_wovyrpkg(envelope.as_bytes());
    }
}
