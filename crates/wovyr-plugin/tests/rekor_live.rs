...
//!
//! Capability-gated: requires the `rekor` cargo feature **and** `WOVYR_REKOR_URL`
...
//! WOVYR_REKOR_URL=http://localhost:3000 \
//!   cargo test -p wovyr-plugin --features rekor --test rekor_live
//! ```
#![cfg(feature = "rekor")]
...
use wovyr_plugin::rekor::RekorLog;

fn rekor_url() -> Option<String> {
    std::env::var("WOVYR_REKOR_URL")
        .ok()
        .filter(|s| !s.is_empty())
}
...
fn keyless_sign_via_rekor_then_offline_verify() {
    let Some(url) = rekor_url() else {
        eprintln!("skipping: WOVYR_REKOR_URL not set");
        return;
    };
