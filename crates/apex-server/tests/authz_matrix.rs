...
//!
//! Compiled as an integration test — a normally-built copy of `apex-server` with no
...
//!
//! Constructed entirely from `apex_server`'s public `AppState` builders
//! (`with_tenancy`/`with_api_keys`/`with_auth_mode`/`with_anonymous_allowed`), so it
//! never touches a real `~/.apex/*` directory or races the crate's own unit tests'
...
/// `crates/apex-server/src/lib.rs`'s `router()` and the sub-route modules it merges**
...
async fn send(state: &Arc<AppState>, case: &RouteCase, auth_header: Option<&str>) -> StatusCode {
    ...
    let resp = apex_server::router(state.clone())
