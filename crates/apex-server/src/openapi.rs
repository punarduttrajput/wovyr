...
/// a bearer credential (`APEX_AUTH_MODE=jwt|apikey`) alongside the always-asserted
...
impl Modify for SecurityAddon {
    fn modify(&self, openapi: &mut utoipa::openapi::OpenApi) {
        ...
        components.add_security_scheme(
            "tenantHeader",
            SecurityScheme::ApiKey(ApiKey::Header(ApiKeyValue::new("X-Apex-Tenant"))),
        );
        components.add_security_scheme(
            "bearerAuth",
...
#[openapi(
    ...
    servers(
        (url = "http://127.0.0.1:8080", description = "Local `apex dev` single-node server"),
    ),
