...
//! programmatic-only MCP client (`apex-tools::mcp`, RM-AIM-P3 ECO-301): add a
...
//! (`APEX_ENABLE_MCP_STDIO=1`, the exact `APEX_ENABLE_SHELL_TOOL` precedent) —
...
use apex_tools::{McpClientCache, McpConnection, McpConnectionStore, McpTransportConfig};
use axum::{
    Json, Router,
...
pub struct McpRuntime {
    ...
    /// `APEX_ENABLE_MCP_STDIO=1` at startup — the same operator-only opt-in
    /// shape `APEX_ENABLE_SHELL_TOOL` uses (SEC-301). Resolved once, not
...
impl McpRuntime {
    ...
    fn stdio_enabled_from_env() -> bool {
        std::env::var("APEX_ENABLE_MCP_STDIO")
            .map(|v| v == "1")
            .unwrap_or(false)
    }

    /// The production runtime: a durable connection store under
    /// `~/.apex/mcp` and the hosted-safety gate read from the environment.
    pub fn from_env() -> apex_common::Result<Self> {
        let dir = apex_config::paths::mcp_dir()?;
        Ok(Self::build(
    ...
    /// `~/.apex/mcp` directory can't be created — ephemeral (lost on
    ...
    pub(crate) fn from_temp_dir_fallback() -> Self {
        let dir = std::env::temp_dir().join(format!("apex_mcp_fallback_{}", std::process::id()));
    ...
    pub fn in_memory() -> Self {
        ...
        let dir =
            std::env::temp_dir().join(format!("apex_mcp_runtime_test_{}_{n}", std::process::id()));
...
fn tool_list_body(tools: Vec<apex_tools::McpToolInfo>) -> Value {
    json!(
        tools
            .into_iter()
...
/// rather than a typed nested schema, since `apex_tools::McpTransportConfig`
...
/// `mcp:admin` scope and the operator's `APEX_ENABLE_MCP_STDIO=1` opt-in
...
pub(crate) async fn create_handler(
    ...
    if transport.is_stdio() && !state.mcp.stdio_enabled() {
        return Err(ApiError::new(
            ...
            "forbidden",
            "Stdio-transport MCP connections require the operator to set APEX_ENABLE_MCP_STDIO=1",
        ));
    }
...
/// the form, whether the operator has set `APEX_ENABLE_MCP_STDIO=1` — PRD-006
...
mod tests {
    ...
    /// runtime (never the real `~/.apex/mcp`) — `stdio_enabled` lets each
...
"#
    ...
    async fn a_stdio_connection_is_refused_without_the_operator_opt_in() {
        ...
        let (status, body) = request(
            ...
            "/api/v1/mcp/connections",
            "root",
            Some(stdio_connection_body("echo", "APEX_TEST_VAR")),
        )
        .await;
    ...
    async fn full_lifecycle_create_list_get_refresh_delete() {
        ...
        let (status, _) = request(
            ...
            "/api/v1/mcp/connections",
            "root",
            Some(stdio_connection_body("fs", "APEX_TEST_VAR")),
        )
        .await;
    ...
    async fn the_env_var_actually_reaches_the_spawned_process() {
        // End-to-end proof (mirroring apex-tools' own MCX-105 test, but through
