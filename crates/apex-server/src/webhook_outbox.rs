...
//! is rewritten via `apex_common::fs::atomic_write` on every mutation, and `path: None`
//! (what most tests use) is a purely in-memory store.

use apex_events::Event;
...
mod tests {
    ...
    fn pending_delivery_survives_reopen() {
        let dir = std::env::temp_dir().join(format!("apex_outbox_pending_{}", std::process::id()));
    ...
    fn dead_letter_is_persisted_and_queryable() {
        let dir = std::env::temp_dir().join(format!("apex_outbox_dlq_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let path = dir.join("outbox.json");
