...
use apex_marketplace::{InMemoryRegistryStore, Registry, SearchQuery};
...
use apex_plugin::{Package, PluginEngine, PluginState, TrustStore};
...
fn keyless_publish_download_install_enable() {
    ...
    .unwrap();
    let apexpkg = Package::new(MANIFEST, Vec::new())
        .with_keyless(bundle)
        .to_apexpkg()
        .unwrap();
    ...
    let out = registry
        .publish(&apexpkg, &["examples".into()], None)
        .unwrap();
    ...
    let package = Package::from_apexpkg(&downloaded).unwrap();
