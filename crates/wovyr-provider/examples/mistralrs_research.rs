...
//! mistral.rs) driving the real `wovyr_agent::run_agent` tool-calling loop,
//! with the real `http_get` builtin tool — no mocks anywhere in this path.
//!
//! Run with (from the workspace root):
//! ```text
//! cargo run -p wovyr-provider --example mistralrs_research --features mistralrs --release -- "your question"
//! ```
//!
