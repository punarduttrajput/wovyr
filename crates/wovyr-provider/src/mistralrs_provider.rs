...
//! tests — **empirically corrected**: `wovyr-eval`'s
//! `real_model_comparison_variance_over_n_runs`
//! (`crates/wovyr-eval/tests/real_model_comparison.rs`) ran the identical
...
//! `wovyr_agent::run_agent`, so there is only ever one agent loop in the
...
impl MistralRsProvider {
    /// Build from the environment: `WOVYR_MISTRALRS_GGUF_REPO` / `_GGUF_FILE` /
    ...
    pub async fn from_env() -> Result<Self> {
        let repo =
            std::env::var("WOVYR_MISTRALRS_GGUF_REPO").unwrap_or_else(|_| DEFAULT_GGUF_REPO.into());
        let file =
            std::env::var("WOVYR_MISTRALRS_GGUF_FILE").unwrap_or_else(|_| DEFAULT_GGUF_FILE.into());
        let tok_model_id = std::env::var("WOVYR_MISTRALRS_TOK_MODEL_ID")
