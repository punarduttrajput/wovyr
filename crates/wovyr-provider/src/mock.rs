...
//! `MockProvider` lets `wovyr agents run --local` work with zero configuration and
...
impl AIProvider for MockProvider {
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let reply = if user.is_empty() {
            "Hello! I'm an Wovyr agent running locally with the mock provider. \
             Set OPENAI_API_KEY to use a real model."
                .to_string()
        } else {
            format!(
                "Hello! I'm an Wovyr agent (mock provider). You said: \"{user}\". \
