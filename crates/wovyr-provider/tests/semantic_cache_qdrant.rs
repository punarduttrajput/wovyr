...
//!
//! Capability-gated like the other backend tests: reads `WOVYR_QDRANT_URL` and skips
...
//!
//! ```bash
//! WOVYR_QDRANT_URL=http://127.0.0.1:6333 \
//!   cargo test -p wovyr-provider --features qdrant --test semantic_cache_qdrant -- --nocapture
//! ```
//!
//! Data is isolated by a per-run nonce `param_key`; a shared collection
//! (`wovyr_it_semcache`) is reused across runs (points keyed off param_key+vector).

#![cfg(feature = "qdrant")]

use std::time::{SystemTime, UNIX_EPOCH};
use wovyr_provider::{ChatResponse, Message, QdrantSemanticCache, SemanticCacheStore};

const COLLECTION: &str = "wovyr_it_semcache";
...
fn cache() -> Option<QdrantSemanticCache> {
    match std::env::var("WOVYR_QDRANT_URL") {
        Ok(url) => Some(QdrantSemanticCache::new(url, COLLECTION)),
        Err(_) => {
            eprintln!("skipping: WOVYR_QDRANT_URL not set");
            None
        }
    }
}
...
fn response(text: &str) -> ChatResponse {
    ChatResponse {
        ...
        model: "m".to_string(),
        usage: wovyr_common::Usage::new(3, 2, 0.01),
