...
//! `WOVYR_REDIS_URL` and returns early (logging a skip) when unset or unreachable, so
...
//!   cargo test -p wovyr-provider --features redis --test redis_breaker -- --nocapture
//! ```
...
use wovyr_provider::{BreakerConfig, CircuitBreaker, RedisKv, SharedCircuitBreaker};
