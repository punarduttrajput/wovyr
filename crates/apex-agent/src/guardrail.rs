...
//! "no safety" isn't one (the same stance as `apex-eval`'s scorers, and unlike
...
mod tests {
    ...
    #[async_trait]
    impl apex_provider::AIProvider for VerdictProvider {
        fn name(&self) -> &str {
            "verdict"
        }
        async fn chat(&self, request: ChatRequest) -> Result<apex_provider::ChatResponse> {
