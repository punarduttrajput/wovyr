...
//! against `apex_tools::builtin`'s hand-written tools (e.g. `FsReadTool`) to see
//! exactly what the derive eliminates.

use apex_tool_macros::Tool;
use apex_tools::{ToolContext, ToolError, ToolRegistry, ToolRequest, ToolResponse};
...
async fn execute_round_trips_typed_parameters_end_to_end() {
    ...
    let resp = registry
        .execute(
            "greet",
            &ctx,
            ToolRequest::new(json!({"name": "Apex", "count": 2})),
        )
        .await
        .unwrap();
    assert!(resp.success);
    assert_eq!(resp.payload["greeting"], "Hello, Apex! Hello, Apex!");

    // The `count` default (via serde, not the derive) applies when omitted.
    let resp = registry
        .execute("greet", &ctx, ToolRequest::new(json!({"name": "Apex"})))
        .await
        .unwrap();
    assert_eq!(resp.payload["greeting"], "Hello, Apex!");
}
