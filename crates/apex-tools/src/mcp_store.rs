...
//! call this store, exactly as `apex-secrets`'s `Vault` separates storage from
//! the access-control layer in front of it.
//!
//! Persistence follows the same shape every other durable store in this
//! workspace uses (`apex-tenancy`'s `FileTenancyStore` is the closest
//! relative): one JSON document per store, guarded by a process-local
//! `Mutex` **and** a cross-process [`apex_common::fs::FileLock`] spanning the
//! whole load→mutate→save cycle (RM-GA-P2 DUR-403), written via
//! [`apex_common::fs::atomic_write`] (DUR-401) so a crash never leaves a torn
//! file.

use apex_common::{Error, Result};
use apex_secrets::{SecretAccess, SecretRef, Vault};
...
pub struct McpConnection {
    ...
    /// value (MCX-105). `apex_secrets::SecretRef` doesn't derive
...
impl McpConnectionsState {
    ...
    /// error — matches `apex-secrets`'s delete semantics).
...
mod tests {
    ...
    async fn connect_injects_the_resolved_secret_as_the_configured_env_var_on_a_real_process() {
        let vault = Vault::new(std::sync::Arc::new(apex_secrets::InMemorySecretStore::new()));
        vault
...
rl.on('line', (line) => {
  ...
  if (msg.method === 'initialize') {
    const resp = {
      ...
      result: { serverInfo: { name: 'echo-env-test', version: process.env.APEX_TEST_WEATHER_KEY || 'MISSING' } },
    };
...
"#;
        ...
        c.secret_ref = Some("secret://acme/weather-api-key".to_string());
        c.secret_env_var = Some("APEX_TEST_WEATHER_KEY".to_string());
