...
//! [`wovyr_ui::UiFrame`]s, between generation and emission.
//!
...
use wovyr_ui::{ActionClass, UiFrame, UiNode};
...
pub struct PolicyRules {
    /// Node budget (protocol cap [`wovyr_ui::MAX_NODES`] still applies above).
    pub max_nodes: usize,
    /// Depth budget (protocol cap [`wovyr_ui::MAX_DEPTH`] still applies above).
    pub max_depth: usize,
...
/// `WOVYR_UNRESTRICTED_UI=1` (resolved by the platform, not here) is the
...
mod tests {
    ...
    fn frame(root: serde_json::Value) -> UiFrame {
        UiFrame::from_value(&json!({
            "schema_version": wovyr_ui::SCHEMA_VERSION,
            "root": root
        }))
