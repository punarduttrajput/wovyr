...
//! `APEX_EVAL_ARTIFACT_DIR` set, so the report/variance/gate JSON persist as
//! build artifacts.
//!
//! To refresh the baseline after an intentional suite/behavior change:
//! `APEX_EVAL_UPDATE_BASELINE=1 cargo test -p apex-eval --test regression_gate`
...
/// Write a JSON artifact into `APEX_EVAL_ARTIFACT_DIR` when set (CI sets it
/// and uploads the directory); a local run without it skips silently.
fn write_artifact(name: &str, value: &impl serde::Serialize) {
    let Ok(dir) = std::env::var("APEX_EVAL_ARTIFACT_DIR") else {
        return;
    };
...
async fn committed_suite_meets_the_committed_baseline_with_zero_variance() {
    ...
    if std::env::var("APEX_EVAL_UPDATE_BASELINE").is_ok() {
