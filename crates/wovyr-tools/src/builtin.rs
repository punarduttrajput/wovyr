...
use wovyr_provider::{Gateway, ImageGenRequest};
...
/// (`ctx.workdir`) — SEC-302. Never able to read outside it (e.g. `~/.wovyr`'s
...
const DEFAULT_USER_AGENT: &str = "Wovyr-AI-Platform/0.1 (+https://github.com/wovyr-ai/wovyr)";
...
/// own bare HTTP client and reading `OPENAI_API_KEY`/`WOVYR_OPENAI_BASE_URL`
/// independently of `wovyr-provider`'s identical env-var contract. Not
...
impl Tool for ImageGenTool {
    ...
    async fn execute(
        ...
        let response = self
            ...
            .await
            .map_err(|e| match e {
                wovyr_common::Error::Invalid(m) | wovyr_common::Error::Config(m) => {
...
const CWD_MARKER: &str = ">>>WOVYR_CWD>>>";
...
fn wrap_with_cwd_marker(command: &str, shell: &str) -> String {
    match shell {
        "powershell" => format!(
            "{command}\n\
             $__wovyrExit = if ($null -ne $LASTEXITCODE) {{ $LASTEXITCODE }} elseif ($?) {{ 0 }} else {{ 1 }}\n\
        ...
        "cmd" => {
            ...
            format!(
                "{command} & set __wovyrExit=!ERRORLEVEL! & echo {escaped_marker}!CD! & exit /b !__wovyrExit!"
            )
        }
        _ => format!(
            "{command}\n\
...
/// backend is selected. Overridable via `WOVYR_SANDBOX_IMAGE`; small + has `/bin/sh`.
...
/// operator choice (`native_only` / `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1`, logged loudly)
...
pub struct ShellTool {
    ...
    /// honors `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1`.
...
impl ShellTool {
    ...
    /// `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1`. Either way an unsandboxed native run logs
    ...
    /// **fails closed** unless the operator sets `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1`
...
/// The container image for isolated shell runs: `WOVYR_SANDBOX_IMAGE` or a default.
fn default_sandbox_image() -> String {
    std::env::var("WOVYR_SANDBOX_IMAGE").unwrap_or_else(|_| DEFAULT_SANDBOX_IMAGE.to_string())
}

/// Whether the operator has explicitly acknowledged an unsandboxed native run via
/// `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1` (SEC-404) — the escape hatch mirroring
/// `WOVYR_ENABLE_SHELL_TOOL`/`WOVYR_ALLOW_ANONYMOUS`. It both permits a native run on
...
fn unsandboxed_native_env_ack() -> bool {
    std::env::var("WOVYR_ALLOW_UNSANDBOXED_NATIVE")
        .map(|v| v == "1")
        .unwrap_or(false)
}
...
///   `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1`), it proceeds
...
async fn resolve_native_confinement(allow_ack: bool) -> Result<NativeConfinement, ToolError> {
    ...
    Err(ToolError::PermissionDenied(
        "refusing to run a native (host-process) tool with no confinement floor: \
         ...
         ~/.wovyr/kms/root.key and exfiltrate it). Run on a Linux host with unprivileged \
         user namespaces for an enforced egress floor, route the run through a \
         container/gVisor backend, or set WOVYR_ALLOW_UNSANDBOXED_NATIVE=1 to explicitly \
    ...
    WARNED.call_once(|| {
        tracing::warn!(
            "running a native (host-process) tool WITHOUT a confinement floor: this \
             ...
             WOVYR_ALLOW_UNSANDBOXED_NATIVE=1). Use a Linux host with unprivileged \
...
/// `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1`, logged loudly) or refused. Selective egress
...
/// `WOVYR_SANDBOX_IMAGE` to a runtime-appropriate image for container/gVisor
...
impl CodeExecuteTool {
    ...
    /// with no floor fails closed unless `WOVYR_ALLOW_UNSANDBOXED_NATIVE=1` (SEC-404).
    pub fn with_manager(manager: SandboxManager) -> Self {
        Self {
            manager,
            image: default_sandbox_image(),
...
impl Tool for CodeExecuteTool {
    ...
    async fn execute(
        ...
        let file_name = format!(".wovyr-codeexec-{}.{ext}", unique_suffix());
...
mod tests {
    ...
    mod tempfile_dir {
        ...
        impl TempDir {
            pub(super) fn new() -> Self {
                ...
                // live: ~1-in-5 runs of `cargo test -p wovyr-tools --lib --
    ...
    /// workspace, `fs_read` cannot reach `~/.wovyr/kms/root.key` regardless of how
    /// the path is spelled.
    #[tokio::test]
    async fn fs_read_cannot_reach_the_kms_root_key() {
        ...
        let kms_key = format!("{home}/.wovyr/kms/root.key");
        let err = FsReadTool
    ...
    async fn image_generate_missing_prompt_is_validation_error() {
        let gateway = Arc::new(Gateway::new(Box::new(wovyr_provider::MockProvider::new())));
    ...
    async fn shell_runs_command_and_reports_success() {
        ...
        let resp = t
            .execute(
                &ctx,
                ToolRequest::new(json!({"command": "echo wovyr_shell_ok"})),
            )
            .await
            .unwrap();
        assert!(resp.success);
    ...
    async fn code_execute_runs_a_node_snippet_and_captures_stdout_and_exit_code() {
        ...
        let resp = t
            .execute(
                ...
                ToolRequest::new(json!({
                    "language": "node",
                    "code": "console.log('wovyr_code_exec_ok')",
                })),
            )
            .await
            .unwrap();
