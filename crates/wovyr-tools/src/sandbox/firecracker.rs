...
/// (whose `/init` is the wovyr guest agent), and the agent executes the command,
...
impl Sandbox for FirecrackerSandbox {
    ...
    async fn execute(&self, cmd: &SandboxCommand) -> Result<CommandOutcome, SandboxError> {
        ...
        let dir = std::env::temp_dir().join(format!("wovyr-fc-{}-{seq}", std::process::id()));
...
/// Parse the guest agent's result blob: `WOVYRR1 / <rc> / b64(stdout) / b64(stderr) /
/// WOVYREOF`, each on its own line, padded with NULs.
fn parse_guest_result(raw: &[u8], max_output: usize) -> Result<CommandOutcome, SandboxError> {
    ...
    if lines.next() != Some(b"WOVYRR1".as_ref()) {
        return Err(bad());
    }
