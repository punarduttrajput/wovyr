...
//! `cargo test -p wovyr-tools --features sandbox-integration-tests -- --ignored`.
...
async fn container_runs_command_and_captures_output() {
    ...
    let out = run(&sb, &cmd("echo", &["wovyr_container_ok"])).await;
    assert_eq!(out.exit_code, Some(0), "stderr: {}", out.stderr);
    assert!(out.stdout.contains("wovyr_container_ok"));
}
...
// correctly-configured host with `cargo test -p wovyr-tools --features
...
async fn firecracker_microvm_runs_command_and_returns_output() {
    ...
    // (carrying the wovyr guest agent as /init), supplied via env.
    if !has(SandboxBackend::Firecracker).await {
        return;
    }
    let (Ok(kernel), Ok(rootfs)) = (
        std::env::var("WOVYR_FC_KERNEL"),
        std::env::var("WOVYR_FC_ROOTFS"),
    ) else {
        eprintln!("skipping: WOVYR_FC_KERNEL / WOVYR_FC_ROOTFS not set");
        return;
    };
    ...
    let mut c = cmd(
        "sh",
        &["-c", "echo wovyr_microvm_ok; cat /etc/alpine-release"],
    );
    c.limits.timeout = Duration::from_secs(30);
    ...
    assert_eq!(out.exit_code, Some(0), "stderr: {}", out.stderr);
    assert!(
        out.stdout.contains("wovyr_microvm_ok"),
...
async fn shell_tool_runs_a_verified_run_in_a_container_not_native() {
    use wovyr_tools::{ShellTool, Tool, ToolContext, ToolRequest, TrustClass};
...
async fn container_read_only_rootfs_denies_writes_outside_workspace() {
    ...
    let out = run(
        ...
        &cmd_in(
            &workdir,
            "sh",
            &["-c", "echo pwned > /etc/wovyr_pwn_test; echo RC=$?"],
        ),
    )
    .await;
