...
//! [`wovyr_secrets::FileSecretStore`](../../wovyr-secrets/src/store.rs) for the
...
pub trait KmsStore: Send + Sync {
    ...
    /// process and another sharing the same `~/.wovyr` directory) could each
    ...
    fn lock(&self) -> Result<Option<wovyr_common::fs::FileLock>> {
        Ok(None)
    }
}
...
mod tests {
    ...
    fn file_store_persists_across_reopen() {
        let dir = std::env::temp_dir().join(format!("wovyr_kms_test_{}", std::process::id()));
    ...
    fn file_store_restricts_kms_json_to_owner_only() {
        let dir = std::env::temp_dir().join(format!("wovyr_kms_perm_test_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let store = FileKmsStore::new(&dir).unwrap();
        store.put(sample("acme")).unwrap();
