...
use wovyr_workflow::{CheckpointStore, Engine, EventLog, InMemoryStore};
...
async fn workflow_app() -> Router {
    use wovyr_workflow::{ClosureExecutor, Definition};
...
async fn async_run_quota_rejection_returns_no_run_id() {
    ...
    let resp = raw(
        ...
        &[
            ("prefer", "respond-async"),
            ("x-wovyr-project", "prj-async-block"),
        ],
        json!({
            "manifest": "metadata:\n  name: q\nspec:\n  instructions: Hi.\n",
            "input": { "message": "hi" }
...
/// A caller-supplied `max_steps` reaches [`wovyr_agent::RunOptions`] rather than
...
async fn metrics_endpoint_reflects_a_run() {
    ...
    assert!(
        text.contains("wovyr_api_requests_total"),
        "metrics:\n{text}"
    );
    ...
    assert!(text.contains("wovyr_api_request_duration_seconds_count"));
    // The mock provider reports a cost, so an LLM cost metric is present.
    assert!(
        text.contains("wovyr_llm_cost_usd_total"),
        "metrics:\n{text}"
    );
    // RM-AIM-P2 OBS-201: per-tenant visibility for both the RED aggregate and the
    // LLM cost/token metrics — none of these requests set `X-Wovyr-Tenant`, so
    // they're all attributed to the "default" tenant.
    assert!(
        text.contains(r#"wovyr_api_requests_by_tenant_total{status_class="2xx",tenant="default"}"#),
        "metrics:\n{text}"
    );
    assert!(
        text.contains(r#"wovyr_llm_cost_usd_by_tenant_total{project="none",tenant="default"}"#),
        "metrics:\n{text}"
    );
    assert!(
        text.contains(r#"wovyr_llm_tokens_by_tenant_total{project="none",tenant="default"}"#),
        "metrics:\n{text}"
    );
}
...
async fn operability_gauges_move_with_load() {
    ...
    for name in [
        "wovyr_async_runs_in_flight",
        "wovyr_webhook_outbox_pending",
        "wovyr_webhook_dlq_size",
        "wovyr_quota_runs_in_flight",
        "wovyr_workflow_timers_pending",
        "wovyr_workflow_executions_active",
    ] {
        assert!(text.contains(&format!("# TYPE {name} gauge")), "{text}");
    }
    let base_runs = gauge(&text, "wovyr_async_runs_in_flight");
    let base_outbox = gauge(&text, "wovyr_webhook_outbox_pending");
    let base_dlq = gauge(&text, "wovyr_webhook_dlq_size");
    let base_timers = gauge(&text, "wovyr_workflow_timers_pending");
    ...
    state
        ...
        .enqueue(crate::webhook_outbox::OutboxEntry {
            ...
            event: wovyr_events::Event::new("evt_g", "project.created", "acme", 1, json!({})),
            enqueued_at_ms: 0,
        });
    state
        .timers
        .schedule(wovyr_workflow::PendingTimer {
    ...
    let text = scrape(&state).await;
    assert_eq!(
        gauge(&text, "wovyr_async_runs_in_flight"),
        base_runs + 1.0,
        "{text}"
    );
    assert_eq!(
        gauge(&text, "wovyr_webhook_outbox_pending"),
        base_outbox + 1.0,
        "{text}"
    );
    assert_eq!(
        gauge(&text, "wovyr_workflow_timers_pending"),
        base_timers + 1.0,
        "{text}"
    );
    ...
    assert_eq!(
        gauge(&text, "wovyr_webhook_dlq_size"),
        base_dlq + 1.0,
        "{text}"
    );
}
...
async fn tenant_req(
    ...
    let resp = raw(
        ...
        method,
        uri,
        &[("x-wovyr-tenant", tenant), ("x-wovyr-principal", principal)],
        body,
    )
    .await;
    let status = resp.status();
...
/// inaccessible to a principal of another tenant, and the `X-Wovyr-Tenant` header
...
/// and a spoofed `X-Wovyr-Tenant` is rejected. (v0.3 exit criterion: zero
...
async fn workflows_are_isolated_per_tenant() {
    use wovyr_tenancy::{MemberScope, Membership, Organization, Role};
    use wovyr_workflow::ClosureExecutor;
...
/// both tenants use the same logical namespace — and `X-Wovyr-Tenant` cannot be
/// spoofed. (v0.3 exit criterion: zero cross-tenant leakage — memory surface.)
#[tokio::test]
async fn memory_is_isolated_per_tenant() {
    use wovyr_memory::{InMemoryStore, MemoryEngine};
...
async fn sensitive_memory_record_is_ciphertext_on_disk_and_plaintext_over_the_api() {
    use wovyr_memory::{EncryptingMemoryStore, FileStore, MemoryEngine, MemoryStore};
    use wovyr_provider::Gateway;
    use wovyr_tenancy::{MemberScope, Membership, Organization, Role};

    let dir = std::env::temp_dir().join(format!(
        "wovyr_server_sensitive_memory_test_{}_{}",
        std::process::id(),
...
async fn audit_route_time_range_and_cursor_page_through_the_window() {
    use wovyr_audit::{AuditEvent, AuditLog};
    use wovyr_tenancy::{MemberScope, Membership, Organization, Role};
...
/// A fresh in-memory-backed KMS for tests, isolated from the shared `~/.wovyr/kms`.
#[cfg(test)]
fn test_kms() -> Arc<dyn wovyr_kms::Kms> {
    Arc::new(wovyr_kms::LocalKms::new(
        wovyr_kms::generate_key().unwrap(),
        Arc::new(wovyr_kms::InMemoryKmsStore::new()),
    ))
}
...
/// RBAC check entirely for a request with no `X-Wovyr-Principal` against the
/// `default` tenant whenever `AppState.anonymous_allowed` — meaning
/// `WOVYR_ALLOW_ANONYMOUS=1` alone let an anonymous caller crypto-shred the
...
async fn anonymous_default_tenant_caller_reaches_kms_admin_only_up_to_the_auth_layer_now() {
    ...
    // No `X-Wovyr-Principal` header, default tenant: authentication passes
    ...
    assert_eq!(
        st,
        StatusCode::FORBIDDEN,
        "anonymity must no longer grant kms:write, even with WOVYR_ALLOW_ANONYMOUS=1"
    );
    ...
    assert_eq!(
        st,
        StatusCode::FORBIDDEN,
        "anonymity must no longer grant kms:admin (crypto-shredding), even with WOVYR_ALLOW_ANONYMOUS=1"
    );
}

/// **SEC-102, production default**: with the anonymous escape hatch disabled (no
/// `WOVYR_ALLOW_ANONYMOUS`), a credential-less request to a protected route is
...
/// `WOVYR_ENABLE_SHELL_TOOL=1` is the explicit operator opt-in (SEC-301) that
...
fn agent_store_survives_a_restart_and_stays_tenant_scoped() {
    let dir =
        std::env::temp_dir().join(format!("wovyr_server_agent_restart_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
...
/// while the server is already at `WOVYR_HTTP_MAX_CONCURRENCY` is refused
...
async fn tls_config_serves_https_end_to_end() {
    ...
    let dir = std::env::temp_dir().join(format!("wovyr-tls-test-{}", std::process::id()));
...
async fn tenant_rate_tier_is_shared_across_principals_and_isolated_by_tenant() {
    ...
    for principal in ["alice", "bob"] {
        let resp = raw(
            ...
            "GET",
            "/api/v1/tools",
            &[("x-wovyr-principal", principal), ("x-wovyr-tenant", "acme")],
            Value::Null,
        )
        .await;
    ...
    let resp = raw(
        ...
        "GET",
        "/api/v1/tools",
        &[("x-wovyr-principal", "carol"), ("x-wovyr-tenant", "acme")],
        Value::Null,
    )
    .await;
    assert_eq!(
    ...
    let resp = raw(
        ...
        "GET",
        "/api/v1/tools",
        &[("x-wovyr-principal", "dave"), ("x-wovyr-tenant", "globex")],
        Value::Null,
    )
    .await;
    assert_eq!(resp.status(), StatusCode::OK);
...
/// (which auth overwrites `X-Wovyr-Principal` with), not a client-supplied header —
            ...
            router(state)
                .oneshot(
                    Request::builder()
                        ...
                        .header("x-wovyr-principal", spoofed_principal)
                        .body(Body::empty())
                        .unwrap(),
                )
                .await
...
/// Default posture (`WOVYR_CORS_ALLOWED_ORIGINS` unset/empty): no `CorsLayer` at
