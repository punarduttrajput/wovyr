...
use wovyr_provider::{CostEvent, CostObserver, Gateway};
use wovyr_tenancy::{FileTenancyStore, InMemoryTenancyStore, TenancyStore};
...
/// a real local model with `WOVYR_PROVIDER=mistralrs` — in which case every agent and
...
pub(crate) async fn default_gateway() -> Gateway {
    let wants_mistralrs = std::env::var("WOVYR_PROVIDER")
    ...
    {
        match wovyr_provider::MistralRsProvider::from_env().await {
            Ok(provider) => return Gateway::new(Box::new(provider)),
            Err(e) => {
                tracing::error!(
                    error = %e,
                    "WOVYR_PROVIDER=mistralrs requested but the model failed to load; \
    ...
    {
        tracing::warn!(
            "WOVYR_PROVIDER=mistralrs requested but this wovyr-server build lacks the \
...
/// A durable [`FileTenancyStore`] at `~/.wovyr/tenancy` (shared with the CLI), falling
/// back to an in-memory store if that directory is unavailable.
pub(crate) fn default_tenancy_store() -> Arc<dyn TenancyStore> {
    if let Ok(dir) = wovyr_config::paths::tenancy_dir()
...
/// shared with the CLI via `wovyr-config` (RM-GA-P4 HLTH-903) so both processes
...
/// `build_kms` fails closed when neither `WOVYR_KMS_ROOT_KEY` nor a persistent,
...
/// up in a data-loss trap; `WOVYR_KMS_ALLOW_EPHEMERAL=1` is the explicit
/// throwaway/test opt-out.
pub(crate) fn default_kms() -> Arc<dyn wovyr_kms::Kms> {
    wovyr_config::kms::build_kms().unwrap_or_else(|e| panic!("refusing to start: {e}"))
}

/// A secret [`Vault`](wovyr_secrets::Vault) over a durable store at
/// `~/.wovyr/secrets` (shared with the CLI via `wovyr-config`, RM-GA-P4
...
/// `WOVYR_SECRETS_PLAINTEXT=1` is the explicit opt-out.
pub(crate) fn default_secrets_vault(kms: Arc<dyn wovyr_kms::Kms>) -> wovyr_secrets::Vault {
    wovyr_config::secrets::build_secrets_vault(kms)
}

/// A tamper-evident [`AuditLog`](wovyr_audit::AuditLog) over a durable [`FileAuditSink`]
/// at `~/.wovyr/audit`, falling back to an in-memory log. Opened with cross-process
/// locking (RM-GA-P2 DUR-403) over that same directory — the CLI can append to the
/// identical `audit.jsonl` (e.g. via `wovyr plugin` commands, once wired), so a
/// second writer must extend the chain, not fork it.
///
/// **Keyed + tamper-evident by default (SEC-403).** The audit MAC key is sourced via
/// `wovyr_config::audit::build_audit_key` (`WOVYR_AUDIT_MAC_KEY`, else a generate-once
/// `~/.wovyr/audit/audit.key`) so the chain is a keyed HMAC an actor with write access
...
/// `WOVYR_AUDIT_ALLOW_UNKEYED=1` is the explicit throwaway/test opt-out (runs the old
/// unkeyed chain).
pub(crate) fn default_audit_log() -> wovyr_audit::AuditLog {
    let dir = wovyr_config::paths::audit_dir().ok();
    let key =
        wovyr_config::audit::build_audit_key().unwrap_or_else(|e| panic!("refusing to start: {e}"));
    let sink = dir
        .clone()
        .and_then(|dir| wovyr_audit::FileAuditSink::new(dir).ok());
    match (sink, dir, key) {
        // Keyed: HMAC chain + durable head anchor over the audit directory.
        (Some(s), Some(dir), Some(k)) => wovyr_audit::AuditLog::open_keyed(Box::new(s), k, dir)
            .unwrap_or_else(|_| wovyr_audit::AuditLog::in_memory()),
        // Explicit unkeyed opt-out: original SHA-256 chain, cross-process locked.
        (Some(s), Some(dir), None) => wovyr_audit::AuditLog::open_with_lock(Box::new(s), dir)
            .unwrap_or_else(|_| wovyr_audit::AuditLog::in_memory()),
        // No durable sink (e.g. no HOME): ephemeral in-memory log.
        _ => wovyr_audit::AuditLog::in_memory(),
    }
}

/// A durable webhook store at `~/.wovyr/webhooks`, falling back to in-memory.
...
/// `webhooks.json`) when `WOVYR_WEBHOOKS_ENCRYPT_AT_REST` is set — **opt-in**,
...
pub(crate) fn default_webhook_store(kms: Arc<dyn wovyr_kms::Kms>) -> Arc<dyn WebhookStore> {
    let dir = wovyr_config::paths::webhooks_dir().ok();
    let encrypt_at_rest = std::env::var("WOVYR_WEBHOOKS_ENCRYPT_AT_REST").is_ok();
    if let Some(dir) = dir {
        if encrypt_at_rest {
...
/// store at `~/.wovyr/mcp`, falling back to a temp-directory-backed store
...
pub(crate) fn default_mcp_runtime() -> crate::mcp::McpRuntime {
    crate::mcp::McpRuntime::from_env().unwrap_or_else(|e| {
        tracing::error!(
            error = %e,
            "could not open the MCP connection store under ~/.wovyr/mcp; \
...
/// An [`Engine`] over the durable workflow store at `~/.wovyr/workflows` (the same
...
/// [`wovyr_runtime::PlatformActivityExecutor`] (RM-GA-P4 HLTH-901), parameterized by
...
pub(crate) fn default_workflows_engine(
        ...
        && let Ok(store) = FileStore::new(dir)
    {
        ...
        let checkpoints: Arc<dyn wovyr_workflow::CheckpointStore> = Arc::new(store);
...
/// A durable [`TimerStore`] at `~/.wovyr/workflows` (shared with the CLI's `wovyr
...
/// A durable [`ScheduleStore`] at `~/.wovyr/workflows` (shared with the CLI's `wovyr
...
/// `~/.wovyr/workflows/definitions` — where `POST /api/v1/workflows` persists the
...
/// `wovyr_workflow::FileStore`'s execution-id sanitization).
...
pub(crate) fn save_definition(workflow_name: &str, manifest: &str) {
    ...
    if let Err(e) = wovyr_common::fs::atomic_write(&path, manifest) {
...
/// start due schedules (G2) without an operator ever running `wovyr workflows tick`
/// (RM-GA-P2 EXE-601), polling every `interval` (`serve()` reads
/// `WOVYR_DISPATCH_INTERVAL_SECS`, default 5s; tests pass a short interval directly
...
/// `~/.wovyr/workflows` — shared with the CLI. Also where the agent store
...
pub(crate) fn workflows_dir() -> Option<PathBuf> {
    wovyr_config::paths::workflows_dir().ok()
}

/// `~/.wovyr/server` — durable state that is server-process-local, never shared with
...
pub(crate) fn server_state_dir() -> Option<PathBuf> {
    wovyr_config::paths::server_state_dir().ok()
}
...
impl CostObserver for MetricsCostObserver {
    fn on_cost(&self, event: CostEvent) {
        ...
        self.metrics.counter_add(
            "wovyr_llm_cost_usd_total",
            &[("model", &event.model)],
            event.cost_usd,
        );
        if event.cache.is_some() {
            self.metrics.counter_add(
                "wovyr_cache_savings_usd_total",
                &[("subsystem", "llm")],
...
pub(crate) struct HttpLimits {
    /// `WOVYR_HTTP_TIMEOUT_SECS` — the default per-request timeout, applied to every
    /// route except the direct agent-run endpoints.
    pub(crate) timeout_secs: u64,
    /// `WOVYR_HTTP_RUN_TIMEOUT_SECS` — a longer timeout for the direct agent-run
    ...
    /// `WOVYR_HTTP_MAX_BODY_BYTES` — the request body size cap; default 1 MiB.
    pub(crate) max_body_bytes: usize,
    /// `WOVYR_HTTP_MAX_CONCURRENCY` — the server-wide in-flight request cap; load past
...
impl HttpLimits {
    pub(crate) fn from_env() -> Self {
        Self {
            timeout_secs: env_u64("WOVYR_HTTP_TIMEOUT_SECS", 30),
            run_timeout_secs: env_u64("WOVYR_HTTP_RUN_TIMEOUT_SECS", 300),
            max_body_bytes: env_u64("WOVYR_HTTP_MAX_BODY_BYTES", 1024 * 1024) as usize,
            max_concurrency: env_u64("WOVYR_HTTP_MAX_CONCURRENCY", 512) as usize,
        }
    }
}
    ...
    Some(
        tower_http::cors::CorsLayer::new()
            ...
            .allow_headers([
                ...
                header::IF_MATCH,
                header::HeaderName::from_static("x-wovyr-tenant"),
                header::HeaderName::from_static("x-wovyr-principal"),
                header::HeaderName::from_static("x-wovyr-project"),
...
/// arrived while the server was already at `WOVYR_HTTP_MAX_CONCURRENCY` is `503` (the
