//! Single-node HTTP server for the Wovyr Platform API.
//!
...
pub fn router(state: Arc<AppState>) -> Router {
    ...
    // deployment configured one (`WOVYR_RATE_LIMIT_TENANT_PER_MIN`).
    ...
    let run_routes = Router::new()
        ...
        // (auth overwrites `X-Wovyr-Principal`) — not a spoofable raw header.
        .layer(idempotency())
...
async fn refresh_operability_gauges(state: &AppState) {
    use wovyr_workflow::ExecutionFilter;
    ...
    m.gauge_set("wovyr_webhook_outbox_pending", &[], outbox_pending as f64);
    m.gauge_set("wovyr_webhook_dlq_size", &[], dlq_size as f64);
    ...
    match state.timers.due(u64::MAX).await {
        Ok(due) => m.gauge_set("wovyr_workflow_timers_pending", &[], due.len() as f64),
    ...
    match state.workflows.list(&ExecutionFilter::default()).await {
        Ok(summaries) => {
            ...
            m.gauge_set("wovyr_workflow_executions_active", &[], active as f64);
        }
...
/// TLS material paths from `WOVYR_TLS_CERT`/`WOVYR_TLS_KEY` (PEM files), if both are
/// configured ([SEC-202](../../docs/18-roadmap/v1.0/phase1-security-floor-tickets.md)).
fn tls_cert_key_paths() -> Option<(String, String)> {
    let cert = std::env::var("WOVYR_TLS_CERT").ok()?;
    let key = std::env::var("WOVYR_TLS_KEY").ok()?;
    Some((cert, key))
}
...
/// (`WOVYR_TLS_TERMINATED_UPSTREAM`) — cleartext HTTP (carrying credentials post-SEC-101
...
fn check_insecure_bind(has_tls: bool, addr: SocketAddr) -> wovyr_common::Result<()> {
    let terminated_upstream = std::env::var_os("WOVYR_TLS_TERMINATED_UPSTREAM").is_some();
    if !has_tls && !terminated_upstream && !addr.ip().is_loopback() {
        return Err(wovyr_common::Error::config(format!(
            "refusing to bind {addr} without TLS: set WOVYR_TLS_CERT and WOVYR_TLS_KEY \
             (PEM files), or WOVYR_TLS_TERMINATED_UPSTREAM=1 if a reverse proxy already \
...
/// in-process rustls acceptor when `WOVYR_TLS_CERT`/`WOVYR_TLS_KEY` are set (SEC-202);
/// otherwise refuses to bind a non-loopback address unless
/// `WOVYR_TLS_TERMINATED_UPSTREAM` declares TLS is handled by a reverse proxy.
pub async fn serve(addr: SocketAddr) -> wovyr_common::Result<()> {
    ...
    // `wovyr workflows tick` on the same host — a `wait: {timer: {after: "30d"}}`
    ...
    // HTTP server itself.
    let dispatch_interval = Duration::from_secs(env_u64("WOVYR_DISPATCH_INTERVAL_SECS", 5));
    ...
    let grace = Duration::from_secs(env_u64("WOVYR_SHUTDOWN_GRACE_SECS", 30));
    ...
    let result = match tls {
        ...
        None => match tokio::net::TcpListener::bind(addr).await {
            Ok(listener) => {
                tracing::info!(%addr, "wovyr server listening (http)");
                serve_http(listener, app, shutdown_signal()).await
            }
            Err(e) => Err(wovyr_common::Error::Io(e)),
        },
    };
    ...
    tracing::info!("wovyr server stopped");
    result
}
...
async fn serve_http(
    ...
    .await
    .map_err(|e| wovyr_common::Error::Runtime(format!("server error: {e}")))
}
...
async fn serve_tls(
    ...
    let config = axum_server::tls_rustls::RustlsConfig::from_pem_file(cert, key)
        .await
        .map_err(|e| wovyr_common::Error::config(format!("failed to load TLS cert/key: {e}")))?;
    tracing::info!(%addr, "wovyr server listening (https)");
