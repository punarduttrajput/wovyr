...
use utoipa::ToSchema;
use wovyr_agent::{AgentDefinition, NullSink, RunEvent, RunEventSink, RunOptions, run_agent};
use wovyr_common::Error;
use wovyr_workflow::{ExecutionFilter, WorkflowState};
...
pub(crate) struct RunRequest {
    ...
    #[serde(default)]
    input: Value,
    /// Override the model/tool iteration cap (default: [`wovyr_agent::RunOptions`]'s).
    #[serde(default)]
...
async fn resolve_run_registry(
    ...
    tenant: &str,
    def: AgentDefinition,
) -> Result<(wovyr_tools::ToolRegistry, AgentDefinition), ApiError> {
