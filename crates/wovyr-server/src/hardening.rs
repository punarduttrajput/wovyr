...
/// `atomic_write` gives a whole-file rewrite, via [`wovyr_common::fs::sync_parent_dir`]
/// — the standalone primitive this workspace already exposes for append-only logs
/// that don't rename anything, the same one `wovyr-workflow`'s event log and
/// `wovyr-audit`'s hash chain use for this exact reason). The log can accumulate more
...
impl IdempotencyStore {
    ...
    fn append_line(path: &PathBuf, record: &PersistedEntry) -> std::io::Result<()> {
        ...
        file.sync_data()?;
        drop(file);
        wovyr_common::fs::sync_parent_dir(path)
    }
    ...
    fn write_snapshot(path: &PathBuf, records: &[PersistedEntry]) -> std::io::Result<()> {
        ...
        wovyr_common::fs::atomic_write(path, out)
    }
}
...
/// routes can never collide. Tenant is read straight off `X-Wovyr-Tenant` (this layer
...
/// caller flooding `X-Wovyr-Tenant`/`X-Wovyr-Project` with arbitrary values — this
...
impl TenantLabelCap {
    ...
    /// an unset `X-Wovyr-Project` — is never bucketed; it renders as `"none"` so a
...
/// existing `wovyr_llm_tokens_total`/`wovyr_llm_cost_usd_total`
/// ([`crate::config::MetricsCostObserver`]) are labeled by `model` only — they come
/// from a [`CostObserver`](wovyr_provider::CostObserver) attached once to the shared
...
/// — the natural point where `tenant`/`project` and the run's [`wovyr_common::Usage`]
...
pub(crate) fn record_llm_usage_metrics(
    ...
    metrics.counter_add(
        "wovyr_llm_cost_usd_by_tenant_total",
        &[("tenant", &tenant), ("project", &project)],
        cost_usd,
    );
    metrics.counter_add(
        "wovyr_llm_tokens_by_tenant_total",
        &[("tenant", &tenant), ("project", &project)],
        tokens as f64,
    );
}
...
/// `agents:run`/`agents/{id}/run` recorded `wovyr_api_requests_total`/
/// `wovyr_api_request_duration_seconds` — via two hand-rolled, near-duplicate
...
///
/// **Per-tenant visibility (RM-AIM-P2 OBS-201):** `wovyr_api_requests_total`/
...
/// aggregate — `wovyr_api_requests_by_tenant_total{tenant, status_class}` — answers
...
/// `X-Wovyr-Tenant` — unverified at this outer layer, same caveat as the tenant
...
pub(crate) async fn track_metrics(
    ...
    metrics.histogram_observe(
        "wovyr_api_request_duration_seconds",
        &[("route", label), ("method", method.as_str())],
        start.elapsed().as_secs_f64(),
    );
    metrics.counter_inc(
        "wovyr_api_requests_by_tenant_total",
        &[
            ("tenant", &tenant),
...
/// `wovyr-workflow`'s `cron.rs::civil_from_days` (same no-dependency
...
mod tests {
    ...
    fn put_appends_instead_of_rewriting_the_whole_log_each_time() {
        let dir =
            std::env::temp_dir().join(format!("wovyr-idempotency-append-{}", std::process::id()));
    ...
    fn put_compacts_the_log_once_it_grows_past_the_threshold() {
        let dir =
            std::env::temp_dir().join(format!("wovyr-idempotency-compact-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("idempotency.jsonl");
    ...
    async fn track_metrics_records_a_normal_response_and_falls_back_for_an_unknown_path() {
        ...
        assert!(
            out.contains(
                r#"wovyr_api_requests_total{method="GET",route="healthz",status="200"} 1"#
            ),
            "got:\n{out}"
        );
        assert!(out.contains(
            "wovyr_api_request_duration_seconds_count{method=\"GET\",route=\"healthz\"} 1"
        ));
        // The new low-cardinality per-tenant aggregate (OBS-201): no `X-Wovyr-Tenant`
        // header on this request, so it's labeled the default tenant.
        assert!(
            out.contains(
                r#"wovyr_api_requests_by_tenant_total{status_class="2xx",tenant="default"} 1"#
            ),
            "got:\n{out}"
        );
        ...
        assert!(
            out.contains(
                r#"wovyr_api_requests_total{method="GET",route="unmatched",status="404"} 1"#
            ),
            "got:\n{out}"
        );
        ...
        assert!(
            out.contains(
                r#"wovyr_api_requests_by_tenant_total{status_class="4xx",tenant="default"} 1"#
            ),
            "got:\n{out}"
        );
    }
    ...
    async fn track_metrics_labels_the_per_tenant_aggregate_from_the_request_header() {
        ...
        assert!(
            out.contains(
                r#"wovyr_api_requests_by_tenant_total{status_class="2xx",tenant="acme"} 1"#
            ),
            "got:\n{out}"
        );
    ...
    fn record_llm_usage_metrics_labels_cost_and_tokens_by_tenant_and_project() {
        ...
        assert!(
            out.contains(
                r#"wovyr_llm_cost_usd_by_tenant_total{project="prj-1",tenant="acme"} 0.5"#
            ),
            "got:\n{out}"
        );
        assert!(
            out.contains(r#"wovyr_llm_tokens_by_tenant_total{project="prj-1",tenant="acme"} 1200"#),
            "got:\n{out}"
        );
        assert!(
            out.contains(
                r#"wovyr_llm_cost_usd_by_tenant_total{project="none",tenant="acme"} 0.25"#
            ),
            "got:\n{out}"
        );
    }
    ...
    fn record_llm_usage_metrics_shares_the_cap_with_track_metrics_bounding() {
        ...
        let out = metrics.render_prometheus();
        assert!(
            out.contains(r#"wovyr_llm_cost_usd_by_tenant_total{project="none",tenant="other"} 1"#),
            "got:\n{out}"
        );
    }
