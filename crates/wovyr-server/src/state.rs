...
pub struct AppState {
    ...
    /// Durable recurring-schedule registry (G2), shared with the CLI's `wovyr
    ...
    /// operator ever running `wovyr workflows tick`.
    ...
    pub(crate) memory: wovyr_memory::MemoryEngine,
    ...
    pub(crate) memory_store: Arc<dyn wovyr_memory::MemoryStore>,
    /// Secret vault backing the `/api/v1/secrets` routes (tenant-scoped).
    pub(crate) secrets: wovyr_secrets::Vault,
    ...
    pub(crate) kms: Arc<dyn wovyr_kms::Kms>,
    ...
    pub(crate) audit: Arc<wovyr_audit::AuditLog>,
    ...
    /// API-key store backing `WOVYR_AUTH_MODE=apikey` ([SEC-101]).
    ...
    /// once at construction from `WOVYR_AUTH_MODE` — see [`auth::AuthMode::from_env`].
    ...
    /// (RM-AIM-P2 SRV-202), enabled by `WOVYR_RATE_LIMIT_TENANT_PER_MIN` — `None`
    ...
    /// `WOVYR_CORS_ALLOWED_ORIGINS` (comma-separated) — an explicit cross-origin
...
impl AppState {
    ...
    /// `WOVYR_PROVIDER=mistralrs` to run every agent/workflow on this node against a
    ...
    pub async fn from_env() -> Self {
        ...
        let metrics = Metrics::with_otlp_export("wovyr");
        ...
        if std::env::var("WOVYR_ENABLE_SHELL_TOOL")
            .map(|v| v == "1")
            .unwrap_or(false)
        {
            ...
            let manager = wovyr_tools::SandboxManager::detect().await;
        ...
        // via `WOVYR_QUOTA_REDIS_URL` when this binary was built with the `redis`
        ...
        // Durable G1/G2 registries, shared with the CLI's `wovyr workflows tick`/
        ...
        Self {
            ...
            idempotency: hardening::IdempotencyStore::new_with_path(
                Duration::from_secs(crate::config::env_u64(
                    "WOVYR_IDEMPOTENCY_TTL_SECS",
                    24 * 60 * 60,
                )),
                crate::config::env_u64("WOVYR_IDEMPOTENCY_MAX_ENTRIES", 10_000) as usize,
            ...
            runs: Arc::new(RunStore::new_with_path(
                Duration::from_secs(crate::config::env_u64("WOVYR_ASYNC_RUN_TTL_SECS", 60 * 60)),
                crate::config::env_u64("WOVYR_ASYNC_RUN_MAX_ENTRIES", 10_000) as usize,
            ...
            // `WOVYR_RATE_LIMIT_REDIS_URL` is set (SRV-201); per-node otherwise.
            rate_limiter_standard: Arc::new(rate_limit::RateLimiter::from_env(
                "standard",
                crate::config::env_u64("WOVYR_RATE_LIMIT_STANDARD_PER_MIN", 300) as u32,
                crate::config::env_u64("WOVYR_RATE_LIMIT_STANDARD_PER_MIN", 300) as u32,
            )),
            rate_limiter_sensitive: Arc::new(rate_limit::RateLimiter::from_env(
                "sensitive",
                crate::config::env_u64("WOVYR_RATE_LIMIT_SENSITIVE_PER_MIN", 30) as u32,
                crate::config::env_u64("WOVYR_RATE_LIMIT_SENSITIVE_PER_MIN", 30) as u32,
            )),
            // Opt-in per-tenant tier (SRV-202): only when the env var is set.
            rate_limiter_tenant: std::env::var("WOVYR_RATE_LIMIT_TENANT_PER_MIN")
                .ok()
            ...
            cors_allowed_origins: std::env::var("WOVYR_CORS_ALLOWED_ORIGINS")
                .unwrap_or_default()
                .split(',')
                .map(str::trim)
                .filter(|s| !s.is_empty())
    ...
    /// otherwise shares the durable `~/.wovyr/server/webhook_outbox.json` across
    ...
    /// instance instead of touching the real `~/.wovyr/mcp`).
    #[cfg(test)]
    ...
    /// Override the tool registry (tests exercise `WOVYR_ENABLE_SHELL_TOOL`'s effect
    ...
    /// touch the shared `~/.wovyr/memory`).
    #[cfg(test)]
    pub(crate) fn with_memory(
        mut self,
        engine: wovyr_memory::MemoryEngine,
        store: Arc<dyn wovyr_memory::MemoryStore>,
    ) -> Self {
        self.memory = engine;
    ...
    #[cfg(test)]
    pub(crate) fn with_secrets(mut self, vault: wovyr_secrets::Vault) -> Self {
        self.secrets = vault;
        self
    }
    ...
    #[cfg(test)]
    pub(crate) fn with_audit(mut self, audit: wovyr_audit::AuditLog) -> Self {
    ...
    #[cfg(test)]
    pub(crate) fn with_audit_arc(mut self, audit: Arc<wovyr_audit::AuditLog>) -> Self {
        self.audit = audit;
        self
    }

    /// Override the platform KMS (tests inject a fresh in-memory-backed instance so
    /// they don't touch the shared `~/.wovyr/kms`, and so tenant-key state starts clean).
    #[cfg(test)]
    pub(crate) fn with_kms(mut self, kms: Arc<dyn wovyr_kms::Kms>) -> Self {
        self.kms = kms;
        self
    }
    ...
    /// (SEC-102), without mutating the process-global `WOVYR_ALLOW_ANONYMOUS` (which
    ...
    /// mutating the process-global `WOVYR_AUTH_MODE` (which every other test in this
    ...
    /// without mutating the process-global `WOVYR_HTTP_*` env vars (which every other
    ...
    /// `WOVYR_RATE_LIMIT_TENANT_PER_MIN`.
    #[cfg(test)]
...
mod run_store_tests {
    ...
    fn running_run_is_reconciled_to_failed_after_restart() {
        let dir = std::env::temp_dir().join(format!("wovyr_async_runs_{}", std::process::id()));
