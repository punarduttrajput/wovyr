...
//! `WOVYR_RATE_LIMIT_REDIS_URL` set, in a shared Redis so a fleet of N nodes
...
impl RateLimiter {
    ...
    /// is compiled with the `redis` feature and `WOVYR_RATE_LIMIT_REDIS_URL` is
    ...
    pub(crate) fn from_env(tier: &str, capacity: u32, per_minute: u32) -> Self {
        ...
        let Ok(url) = std::env::var("WOVYR_RATE_LIMIT_REDIS_URL") else {
            return limiter;
        };
        #[cfg(feature = "redis")]
        {
            match redis::Client::open(url.as_str()) {
                Ok(client) => {
                    ...
                    return limiter.with_redis(client, format!("wovyr:rl:{tier}"));
                }
                Err(e) => {
                    tracing::error!(
                        tier,
                        error = %e,
                        "WOVYR_RATE_LIMIT_REDIS_URL is invalid; falling back to per-node rate limiting"
                    );
                }
            }
        }
        #[cfg(not(feature = "redis"))]
        {
            ...
            tracing::error!(
                tier,
                "WOVYR_RATE_LIMIT_REDIS_URL is set but this binary was built without the \
...
/// [`crate::auth::authenticate`] (`X-Wovyr-Principal`, overwritten there — never a raw
...
/// The tenant-tier key (RM-AIM-P2 SRV-202): the asserted `X-Wovyr-Tenant`, falling
...
/// bucket and, when the deployment configured one (`WOVYR_RATE_LIMIT_TENANT_PER_MIN`,
...
mod tests {
    ...
    async fn unreachable_redis_degrades_to_local_limiting_not_unlimited() {
        ...
        let limiter = RateLimiter::new(1, 1).with_redis(client, "wovyr:rl:test-degrade");
...
/// capability-gated like `wovyr-provider`'s `redis_breaker` test: they read
/// `WOVYR_REDIS_URL` and return early (logging a `skipping:` line CI's
...
///   cargo test -p wovyr-server --features redis --lib rate_limit -- --nocapture
/// ```
