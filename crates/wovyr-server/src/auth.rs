...
//! `X-Wovyr-Principal` (or a bearer token) was trusted as an unverified, caller-supplied
//! string ([`tenancy`](crate::tenancy) derives principal/tenant from headers alone).
//!
//! Selected by `WOVYR_AUTH_MODE` (default `disabled-loopback`):
//! - `jwt` — a bearer JWT, HS256 (`WOVYR_JWT_HS_SECRET`) or RS256
//!   (`WOVYR_JWT_RS_PUBLIC_KEY`, PEM), checked for expiry always and issuer/audience
//!   (`WOVYR_JWT_ISSUER`/`WOVYR_JWT_AUDIENCE`) when configured. The verified `sub` claim
...
//!
//! A verified credential's principal **overwrites** the request's `X-Wovyr-Principal`
...
/// Which credential scheme the server verifies, from `WOVYR_AUTH_MODE`. Resolved
...
impl AuthMode {
    pub(crate) fn from_env() -> Self {
        match std::env::var("WOVYR_AUTH_MODE").ok().as_deref() {
...
///
/// Explicit opt-in (`WOVYR_ALLOW_ANONYMOUS=1`) in a real deployment. Defaults to
...
pub(crate) fn resolve_anonymous_allowed() -> bool {
    match std::env::var("WOVYR_ALLOW_ANONYMOUS") {
        Ok(v) => v == "1",
        Err(_) => cfg!(test),
    }
}

/// The raw `WOVYR_ALLOW_ANONYMOUS` flag with no test-only default — used only at the
...
fn env_allow_anonymous() -> bool {
    std::env::var("WOVYR_ALLOW_ANONYMOUS")
        .map(|v| v == "1")
        .unwrap_or(false)
}

/// Refuse to bind `addr` when the anonymous escape hatch is enabled on a non-loopback
/// address (SEC-102): `WOVYR_ALLOW_ANONYMOUS=1` is a dev-only convenience, never a
...
/// unit-testable without mutating the process-global `WOVYR_ALLOW_ANONYMOUS` (which
/// every other test in this crate's default-anonymous-in-`cfg(test)` behavior depends
/// on, and would otherwise race against).
fn check_anonymous_bind(allowed: bool, addr: std::net::SocketAddr) -> wovyr_common::Result<()> {
    if allowed && !addr.ip().is_loopback() {
        return Err(wovyr_common::Error::config(format!(
            "WOVYR_ALLOW_ANONYMOUS=1 is refused on a non-loopback bind ({addr}); set \
             WOVYR_AUTH_MODE=jwt or apikey (and unset WOVYR_ALLOW_ANONYMOUS) for a \
...
/// Overwrite (never merge with) the request's `X-Wovyr-Principal` header with the
...
fn set_verified_principal(req: &mut Request, principal: &str) {
    match HeaderValue::from_str(principal) {
        Ok(value) => {
            req.headers_mut().insert("x-wovyr-principal", value);
        }
        Err(_) => {
            req.headers_mut().remove("x-wovyr-principal");
        }
    }
}
    ...
    match state.auth_mode {
        AuthMode::DisabledLoopback => {
            if state.anonymous_allowed {
                ...
                unauthorized(
                    "no credential presented; set WOVYR_AUTH_MODE=jwt|apikey or \
                     WOVYR_ALLOW_ANONYMOUS=1 for local/dev use",
                )
            }
        }
...
fn jwt_decoding_key() -> Result<jsonwebtoken::DecodingKey, String> {
    if let Ok(secret) = std::env::var("WOVYR_JWT_HS_SECRET") {
        return Ok(jsonwebtoken::DecodingKey::from_secret(secret.as_bytes()));
    }
    if let Ok(pem) = std::env::var("WOVYR_JWT_RS_PUBLIC_KEY") {
        return jsonwebtoken::DecodingKey::from_rsa_pem(pem.as_bytes()).map_err(|e| e.to_string());
    }
    Err(
        "WOVYR_AUTH_MODE=jwt requires WOVYR_JWT_HS_SECRET (HS256) or WOVYR_JWT_RS_PUBLIC_KEY \
         (RS256, PEM)"
            .to_string(),
    )
}
...
fn jwt_validation() -> jsonwebtoken::Validation {
    let alg = if std::env::var_os("WOVYR_JWT_RS_PUBLIC_KEY").is_some() {
        jsonwebtoken::Algorithm::RS256
    } else {
        jsonwebtoken::Algorithm::HS256
    };
    let mut validation = jsonwebtoken::Validation::new(alg);
    if let Ok(iss) = std::env::var("WOVYR_JWT_ISSUER") {
        validation.set_issuer(&[iss]);
    }
    if let Ok(aud) = std::env::var("WOVYR_JWT_AUDIENCE") {
...
/// credential without touching `~/.wovyr/auth`.
#[derive(Default)]
...
/// another one sharing the same `~/.wovyr/auth`).
struct CachedKeys {
    mtime: std::time::SystemTime,
    map: BTreeMap<String, KeyRecord>,
}

/// A durable key store at `~/.wovyr/auth/api_keys.json` ([`KeyRecord`] keyed by hash;
...
/// rotate-with-grace — used by the CLI's `wovyr auth` subcommands. **Cached in memory**
...
/// The server's API-key store: durable at `~/.wovyr/auth` (shared with the CLI's own
/// key-minting command), falling back to an empty in-memory store.
pub(crate) fn default_api_key_store() -> Arc<dyn ApiKeyStore> {
    if let Ok(dir) = wovyr_config::paths::auth_dir()
...
mod tests {
    ...
    /// Serializes the tests below: they mutate process-global `WOVYR_JWT_*` env vars,
    ...
    fn jwt_hs256_accepts_a_valid_token_and_extracts_the_principal() {
        ...
        unsafe {
            std::env::set_var("WOVYR_JWT_HS_SECRET", "test-secret-1");
            std::env::remove_var("WOVYR_JWT_RS_PUBLIC_KEY");
            std::env::remove_var("WOVYR_JWT_ISSUER");
            std::env::remove_var("WOVYR_JWT_AUDIENCE");
        }
    ...
    fn jwt_rejects_wrong_signature() {
        let _guard = ENV_LOCK.lock().unwrap();
        unsafe {
            std::env::set_var("WOVYR_JWT_HS_SECRET", "test-secret-2");
            std::env::remove_var("WOVYR_JWT_RS_PUBLIC_KEY");
            std::env::remove_var("WOVYR_JWT_ISSUER");
            std::env::remove_var("WOVYR_JWT_AUDIENCE");
        }
    ...
    fn jwt_rejects_expired_token() {
        let _guard = ENV_LOCK.lock().unwrap();
        unsafe {
            std::env::set_var("WOVYR_JWT_HS_SECRET", "test-secret-3");
            std::env::remove_var("WOVYR_JWT_RS_PUBLIC_KEY");
            std::env::remove_var("WOVYR_JWT_ISSUER");
            std::env::remove_var("WOVYR_JWT_AUDIENCE");
        }
    ...
    fn jwt_rejects_wrong_issuer_and_audience() {
        let _guard = ENV_LOCK.lock().unwrap();
        unsafe {
            std::env::set_var("WOVYR_JWT_HS_SECRET", "test-secret-4");
            std::env::remove_var("WOVYR_JWT_RS_PUBLIC_KEY");
            std::env::set_var("WOVYR_JWT_ISSUER", "wovyr-issuer");
            std::env::set_var("WOVYR_JWT_AUDIENCE", "wovyr-clients");
        }
        let bad_issuer = token_with(
            "test-secret-4",
            json!({ "sub": "alice", "exp": future_exp(), "iss": "someone-else", "aud": "wovyr-clients" }),
        );
        assert!(verify_jwt_bearer(&bad_issuer).is_err());

        let bad_audience = token_with(
            "test-secret-4",
            json!({ "sub": "alice", "exp": future_exp(), "iss": "wovyr-issuer", "aud": "someone-else" }),
        );
        assert!(verify_jwt_bearer(&bad_audience).is_err());

        let good = token_with(
            "test-secret-4",
            json!({ "sub": "alice", "exp": future_exp(), "iss": "wovyr-issuer", "aud": "wovyr-clients" }),
        );
    ...
    fn file_api_key_store_persists_minted_keys() {
        let dir = std::env::temp_dir().join(format!("wovyr-auth-test-{}", std::process::id()));
    ...
    fn file_api_key_store_avoids_repeat_reads_after_warm_up() {
        let dir = std::env::temp_dir().join(format!("wovyr-auth-cache-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let store = FileApiKeyStore::new(&dir).unwrap();
    ...
    /// `wovyr auth` commands) is still picked up — the cache invalidates on the
    /// file's `mtime`, not just this process's own writes.
    #[test]
    fn file_api_key_store_picks_up_an_external_change() {
        let dir = std::env::temp_dir().join(format!("wovyr-auth-external-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn expired_key_is_rejected() {
        let dir = std::env::temp_dir().join(format!("wovyr-auth-exp-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let store = FileApiKeyStore::new(&dir).unwrap();
    ...
    fn revoked_key_is_rejected() {
        let dir = std::env::temp_dir().join(format!("wovyr-auth-rev-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let store = FileApiKeyStore::new(&dir).unwrap();
    ...
    fn rotation_issues_new_key_and_expires_old_after_grace() {
        let dir = std::env::temp_dir().join(format!("wovyr-auth-rot-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let store = FileApiKeyStore::new(&dir).unwrap();
    ...
    fn legacy_hash_to_principal_format_is_migrated() {
        let dir = std::env::temp_dir().join(format!("wovyr-auth-mig-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    /// End-to-end (SEC-101 acceptance criteria): with `WOVYR_AUTH_MODE=apikey`, a
    /// request with no credential, or an unknown one, is `401`; a request bearing a
    /// minted key resolves the correct principal — and a spoofed `X-Wovyr-Principal`
    /// header is powerless to override it, since the middleware overwrites it before
    /// any handler runs. Uses a real tenancy membership (not `WOVYR_PLATFORM_ADMINS`,
    ...
    async fn apikey_mode_end_to_end_over_the_router() {
        ...
        let call = |auth_header: Option<&'static str>, spoof_principal: bool| {
            ...
            async move {
                ...
                if spoof_principal {
                    builder = builder.header("x-wovyr-principal", "alice");
                }
