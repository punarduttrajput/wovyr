...
//! declarative boilerplate an [`wovyr_tools::Tool`] implementation needs — a
//! [`ToolMetadata`](wovyr_tools::ToolMetadata) constructor and a JSON Schema (via
...
//! use wovyr_tools::{ToolContext, ToolError, ToolRequest, ToolResponse};
...
pub fn derive_tool(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics #ident #ty_generics #where_clause {
            ...
            /// should delegate to.
            pub fn __tool_metadata() -> ::wovyr_tools::ToolMetadata {
                ::wovyr_tools::ToolMetadata::new(#id, #version, #category, #description)
            ...
            pub fn __tool_parse_params(
                request: &::wovyr_tools::ToolRequest,
            ) -> ::std::result::Result<#params, ::wovyr_tools::ToolError> {
                ::serde_json::from_value(request.parameters.clone()).map_err(|e| {
                    ::wovyr_tools::ToolError::Validation(format!("invalid parameters: {e}"))
                })
            }
        }
    };
