//! Stage 2: the *full* Wovyr agent loop calling a tool proxied from a real,
...
//!
//! Run: `cargo run -p wovyr-agent --example mcp_filesystem_agent_demo -- <allowed-dir> <file-name>`
...
use wovyr_tools::{McpClient, StdioTransport, ToolRegistry};
...
async fn main() {
    ...
    assert!(
        out.text.contains("hello from wovyr mcp test"),
    ...
    println!(
        "\n\u{2705} End-to-end verified: a real npx-spawned MCP server's tool was discovered, \
         registered, permission-checked, and called by the real Wovyr agent loop, and its real \
