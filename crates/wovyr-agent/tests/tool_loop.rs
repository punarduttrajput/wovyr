...
use wovyr_provider::{AIProvider, ChatRequest, ChatResponse, Gateway, Message, Role, ToolCall};
use wovyr_tools::{
...
/// `WOVYR_UNRESTRICTED_TOOLS` env var, so they can't race each other. `tokio::sync`
...
impl RunEventSink for Capture {
    fn emit(&mut self, event: RunEvent<'_>) {
        let line = match event {
            ...
            RunEvent::ToolCallDelta { name, .. } => format!("tooldelta:{name}"),
...
async fn run_loop_terminates_on_step_budget() {
    ...
    let err = run_agent(&def, &gateway, &registry, opts, &mut Capture::default())
        .await
        .unwrap_err();
    assert!(matches!(err, wovyr_common::Error::Runtime(_)));
}
...
/// `WOVYR_UNRESTRICTED_TOOLS=1` restores the old unrestricted behavior even for a
...
async fn unrestricted_tools_escape_hatch_restores_old_behavior_for_hosted_runs() {
    let _guard = UNRESTRICTED_TOOLS_ENV_LOCK.lock().await;
    unsafe { std::env::set_var("WOVYR_UNRESTRICTED_TOOLS", "1") };
    ...
    unsafe { std::env::remove_var("WOVYR_UNRESTRICTED_TOOLS") };
