//! Sourcing the **audit MAC key** (SEC-403), shared by `apex-server` (and any
//! future CLI audit surface) so both agree on the key that authenticates the
//! tamper-evident audit chain — reusing `apex-kms`'s root-key sourcing
//! (`from_env`/`from_file`) exactly as `apex_config::kms` does.
//!
...
//! `APEX_AUDIT_MAC_KEY` (hex, preferred — forces the operator to source it from
//! escrow before startup) or a generate-once `~/.apex/audit/audit.key`.
//!
//! **Fail-closed on missing key material (SEC-403, the SEC-405 stance).** When
//! *neither* `APEX_AUDIT_MAC_KEY` is set *nor* a persistent, writable key file
...
//! hatch for a genuine throwaway/test run is `APEX_AUDIT_ALLOW_UNKEYED=1` — a
//! deliberate operator choice, mirroring `APEX_KMS_ALLOW_EPHEMERAL` /
//! `APEX_ALLOW_ANONYMOUS`.
//!
...
use apex_kms::KeyBytes;
use std::path::PathBuf;

/// Whether the operator has explicitly opted into running the audit log
/// **unkeyed** (no tamper resistance) via `APEX_AUDIT_ALLOW_UNKEYED=1`.
fn unkeyed_allowed() -> bool {
    std::env::var("APEX_AUDIT_ALLOW_UNKEYED")
        .map(|v| v == "1")
        .unwrap_or(false)
}

/// Resolve the audit MAC key (SEC-403): `APEX_AUDIT_MAC_KEY` (hex) or a
/// generate-once `~/.apex/audit/audit.key`.
///
/// Returns `Ok(Some(key))` for a keyed (tamper-resistant) log, `Ok(None)` when
/// the operator has explicitly opted into an unkeyed log
/// (`APEX_AUDIT_ALLOW_UNKEYED=1`), and [`Error::Config`] when no key material is
...
pub fn build_audit_key() -> Result<Option<KeyBytes>> {
    ...
    let env_key = apex_kms::root::from_env("APEX_AUDIT_MAC_KEY").ok();
...
/// `APEX_AUDIT_MAC_KEY`/`HOME` environment (which concurrent tests share).
fn build_audit_key_inner(
    ...
    let key = env_key.or_else(|| {
        dir.as_ref()
            .and_then(|d| apex_kms::root::from_file(d.join("audit.key")).ok())
    });
    ...
    if allow_unkeyed {
        tracing::warn!(
            "APEX_AUDIT_ALLOW_UNKEYED=1: the audit log runs with an UNKEYED hash chain — \
    ...
    Err(Error::config(
        "no durable audit MAC key available: set APEX_AUDIT_MAC_KEY (hex-encoded 32 bytes) \
         or ensure ~/.apex/audit is writable so a key can be persisted. Refusing to start \
         with an unkeyed audit chain that an actor with write access could silently forge. \
         Set APEX_AUDIT_ALLOW_UNKEYED=1 only for a throwaway/test run (SEC-403).",
    ))
}
...
mod tests {
    ...
    fn missing_key_material_fails_closed() {
        ...
        match build_audit_key_inner(None, None, false) {
            Err(Error::Config(msg)) => assert!(
                msg.contains("APEX_AUDIT_MAC_KEY"),
    ...
    fn unkeyed_opt_in_yields_no_key() {
        ...
        assert_eq!(
            ...
            None,
            "APEX_AUDIT_ALLOW_UNKEYED=1 should run unkeyed, not fail"
        );
    }

    #[test]
    fn env_key_yields_a_keyed_log() {
        let key = apex_kms::generate_key().unwrap();
        assert_eq!(
    ...
    fn a_persistent_directory_yields_a_generated_key() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "apex_config_audit_test_{}_persist",
            std::process::id()
        ));
