...
use apex_kms::{FileKmsStore, InMemoryKmsStore, KeyBytes, Kms, KmsStore, LocalKms};
use std::path::PathBuf;
use std::sync::Arc;

/// Whether the operator has explicitly opted into an **ephemeral** (in-process,
/// non-durable) KMS root key via `APEX_KMS_ALLOW_EPHEMERAL=1` — the documented
/// escape hatch for a genuine test/dev run (SEC-405), mirroring the
/// `APEX_ENABLE_SHELL_TOOL`/`APEX_ALLOW_ANONYMOUS` opt-in precedent. Off means
/// missing durable key material is a fail-closed startup error, not a silent
/// data-loss trap.
fn ephemeral_allowed() -> bool {
    std::env::var("APEX_KMS_ALLOW_EPHEMERAL")
        .map(|v| v == "1")
        .unwrap_or(false)
}
...
/// *neither* `APEX_KMS_ROOT_KEY` is set *nor* a persistent, writable key file
...
/// throwaway/test run is `APEX_KMS_ALLOW_EPHEMERAL=1` — a deliberate operator
/// choice, the `APEX_ALLOW_ANONYMOUS` precedent.
pub fn build_kms() -> Result<Arc<dyn Kms>> {
    let dir = crate::paths::kms_dir().ok();
    let env_key = apex_kms::root::from_env("APEX_KMS_ROOT_KEY").ok();
...
/// `HOME`/`APEX_KMS_ROOT_KEY` environment (which concurrent tests share).
fn build_kms_inner(
    dir: Option<PathBuf>,
    ...
    if allow_ephemeral {
        tracing::warn!(
            "APEX_KMS_ALLOW_EPHEMERAL=1: using an ephemeral in-process KMS root key — \
    ...
    Err(Error::config(
        "no durable KMS root key available: set APEX_KMS_ROOT_KEY (hex-encoded 32 bytes) \
         or ensure ~/.apex/kms is writable so a root key can be persisted. Refusing to \
         start with an ephemeral key that would silently lose every sealed secret and \
         sensitive memory on the next restart. Set APEX_KMS_ALLOW_EPHEMERAL=1 only for a \
    ...
    fn missing_key_material_fails_closed() {
        ...
        match build_kms_inner(None, None, false) {
            Err(Error::Config(msg)) => assert!(
                msg.contains("APEX_KMS_ROOT_KEY"),
