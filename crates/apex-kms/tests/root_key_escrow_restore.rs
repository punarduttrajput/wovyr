//! RM-GA-P2 DR-1002 acceptance: root-key escrow (`APEX_KMS_ROOT_KEY`) plus a
...
//! catalog directory is copied the same way `apex admin backup`/`restore`
//! (DR-1001) would copy `~/.apex/kms`, and the root key is round-tripped
//! through the exact `APEX_KMS_ROOT_KEY`-shaped env var `root::from_env`
//! reads in production.

use apex_kms::envelope::{open, seal};
use apex_kms::{FileKmsStore, KmsStore, LocalKms, generate_key, root};
use std::sync::Arc;

fn scratch_dir(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "apex_kms_escrow_restore_{name}_{}",
        std::process::id()
    ));
...
/// way `apex admin backup` does: lock-machinery state, not data.
...
fn escrowed_root_key_plus_restored_catalog_decrypts_data_sealed_by_a_gone_instance() {
    ...
    // `apex admin backup`, stood in here by a plain copy since apex-kms has no
    ...
    // production does (`APEX_KMS_ROOT_KEY`, DR-1002's documented mandatory
    // mode) and opens the restored catalog. ---
    const ENV_VAR: &str = "APEX_KMS_ROOT_KEY_DR1002_TEST";
