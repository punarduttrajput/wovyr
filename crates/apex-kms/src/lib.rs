...
//!
//! Depends only on `apex-common`, keeping the workspace dependency spine
//! one-directional. Wiring this into a real consumer (`apex-memory`'s
//! sensitive-record flag, `apex-secrets`), server routes/CLI commands, and
//! `apex-audit` integration for key lifecycle events are later slices.
