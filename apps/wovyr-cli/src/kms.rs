//! `wovyr kms` commands: manage the local platform KMS's tenant keys
//! ([Encryption §5](../../docs/13-security/encryption.md#5-key-management)).
//!
//! Operates directly on the same `~/.wovyr/kms` catalog the server uses — no server
//! process required, matching how `memory`/`plugin` commands work locally.

use crate::config;

/// `wovyr kms rotate --tenant <t>` — roll a new tenant-key version. Existing wrapped
/// data keys remain valid under their original version; nothing is re-encrypted.
pub fn rotate_cmd(tenant: &str) -> wovyr_common::Result<()> {
    let version = config::kms().rotate_tenant_key(tenant)?;
    println!("tenant `{tenant}` rotated to key version {version}");
    Ok(())
}

/// `wovyr kms destroy --tenant <t> --yes` — permanently **crypto-shred** a tenant's key
...
pub fn destroy_cmd(tenant: &str, confirmed: bool) -> wovyr_common::Result<()> {
    if !confirmed {
        eprintln!(
        ...
        return Err(wovyr_common::Error::invalid("missing --yes confirmation"));
    }
