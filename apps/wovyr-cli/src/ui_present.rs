//! An interactive, stdin-based [`UiInteraction`] (PRD-005 HIL-304) for `wovyr
...
use std::io::Write;
use wovyr_tools::UiInteraction;
use wovyr_ui::{UiDecision, UiFrame, UiNode};
...
impl UiInteraction for StdinUiPresenter {
    async fn present(&self, frame: &UiFrame) -> wovyr_common::Result<UiDecision> {
        ...
        tokio::task::spawn_blocking(move || present_blocking(&frame))
            .await
            .map_err(|e| wovyr_common::Error::Runtime(format!("ui prompt task panicked: {e}")))?
    }
}

fn present_blocking(frame: &UiFrame) -> wovyr_common::Result<UiDecision> {
    println!();
    println!("--- ui frame ---------------------------------------------");
    ...
    if actions.is_empty() {
        return Err(wovyr_common::Error::Runtime(
...
fn prompt_inputs(node: &UiNode, values: &mut BTreeMap<String, Value>) -> wovyr_common::Result<()> {
    match node {
        UiNode::Column { children } | UiNode::Row { children } => {
            for child in children {
...
fn prompt_line(label: &str, required: bool) -> wovyr_common::Result<Option<String>> {
    loop {
        print!("{label}: ");
        std::io::stdout().flush()?;
        let mut line = String::new();
