//! `wovyr admin backup` / `wovyr admin restore` — snapshot and restore the local
//! `~/.wovyr` state directory (RM-GA-P2 DR-1001).
//!
//! No backup/restore tooling existed anywhere before this: no CLI command, no
//! `pg_dump` hook, no `~/.wovyr` snapshot — and a naive `tar` of a live directory
...
//! explicit source/destination paths rather than reading `~/.wovyr` itself, so
...
/// Named to make an accidental collision with a real `~/.wovyr` entry
/// essentially impossible.
const MANIFEST_FILE: &str = "wovyr_backup_manifest.json";
...
struct Manifest {
    /// The `wovyr-cli` crate version that produced this backup — informational
...
/// `wovyr admin backup <dest>` — snapshot `~/.wovyr` into `<dest>`: a local
...
/// `backup_cmd`, which always resolves this to the real `~/.wovyr` via
...
/// `wovyr admin restore <src> --yes` — restore `~/.wovyr` from a backup made by
/// `wovyr admin backup`: a local directory, or an `s3://bucket/prefix` URI
/// matching a remote backup's destination. Overwrites the live `~/.wovyr` —
...
pub async fn restore_cmd(src: &str, confirmed: bool) -> Result<()> {
    if !confirmed {
        eprintln!(
            "refusing to restore without --yes: this OVERWRITES the live ~/.wovyr \
...
/// `~/.wovyr`, but a test needs a scratch directory instead.
...
/// flaky-test collision elsewhere in this workspace (`wovyr-tools`'
...
async fn run_workflow_migrations(database_url: &str) -> Result<()> {
    wovyr_workflow::PostgresStore::run_migrations(database_url).await?;
...
async fn run_memory_migrations(database_url: &str) -> Result<()> {
    wovyr_memory::PostgresStore::run_migrations(database_url).await?;
...
fn backup_dir(source: &Path, dest: &Path) -> Result<usize> {
    ...
    let manifest = Manifest {
        wovyr_version: env!("CARGO_PKG_VERSION").to_string(),
        files,
    };
    fs::write(
...
fn restore_dir(src: &Path, dest: &Path) -> Result<usize> {
    ...
    for (rel, bytes) in &validated {
        ...
        wovyr_common::fs::atomic_write(&to, bytes)?;
    }
...
/// `~/.wovyr/<store>` locks independently, DUR-403). Generic over whatever
...
mod tests {
    ...
    fn scratch_dir(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("wovyr_cli_admin_{name}_{}", std::process::id()));
    ...
    /// skip cleanly when `WOVYR_S3_ENDPOINT` isn't set, so this suite still passes
    ...
    async fn s3_backup_restore_round_trips_against_a_live_endpoint() {
        if std::env::var("WOVYR_S3_ENDPOINT").is_err() {
            eprintln!("skipping: WOVYR_S3_ENDPOINT not set");
            return;
        }
        // `WOVYR_S3_TEST_BUCKET` must already exist (this client only ever does
        ...
        let bucket = std::env::var("WOVYR_S3_TEST_BUCKET")
            .unwrap_or_else(|_| "wovyr-ci-backups".to_string());
