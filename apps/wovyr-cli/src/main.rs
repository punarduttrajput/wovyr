//! `wovyr` — the Wovyr AI Platform command-line interface.
//!
...
//! wovyr agents run --local -f agents/hello.yaml --input '{"message":"Hi"}' --stream
//! wovyr dev                                  # start a single-node server
//! wovyr agents run -f agents/hello.yaml --input '{"message":"Hi"}'  # remote run
//! ```
//!
...
use wovyr_agent::{AgentDefinition, NullSink, RunOptions, run_agent, run_agent_with_memory};
use wovyr_provider::Gateway;
use wovyr_tools::ToolRegistry;

/// Top-level CLI.
#[derive(Parser)]
#[command(name = "wovyr", version, about = "Wovyr AI Platform CLI", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    Login {
        /// Server base URL (e.g. https://api.wovyr.example.com).
        #[arg(long)]
        server: String,

        /// Access token. Falls back to the WOVYR_TOKEN environment variable.
        #[arg(long)]
        token: Option<String>,
    },
...
#[derive(Subcommand)]
enum AdminCommand {
    /// Snapshot `~/.wovyr` into `<dest>` (agents, secrets, memory, workflows,
    ...
    Backup {
        ...
        /// (needs `WOVYR_S3_ENDPOINT`/`WOVYR_S3_ACCESS_KEY_ID`/`WOVYR_S3_SECRET_ACCESS_KEY`,
        /// and optionally `WOVYR_S3_REGION`).
        dest: String,
    },

    /// Restore `~/.wovyr` from a backup made by `wovyr admin backup`. Overwrites
    /// the live `~/.wovyr` — irreversible for anything written there since the
    /// backup was taken.
    Restore {
        /// Source backup directory, as produced by `wovyr admin backup`, or an
...
enum PluginCommand {
    ...
    /// (digests are computed by `wovyr plugin build`, never hand-edited).
    New {
    ...
    Build {
        /// Plugin project directory (from `wovyr plugin new`).
        #[arg(default_value = ".")]
        project: String,
    ...
    /// (`~/.wovyr/plugins/keyless.json`, ADR-0009).
    KeylessInit {
    ...
    KeylessSign {
        ...
        /// CA key path (defaults to `~/.wovyr/plugins/keyless-ca.key`).
        #[arg(long)]
    ...
    Pack {
        ...
        /// Output file (defaults to `<name>-<version>.wovyrpkg`).
        #[arg(long)]
        out: Option<String>,
    },
    ...
    Publish {
        /// Package directory (required with `--key`) or a pre-signed `.wovyrpkg` file.
        source: String,
...
enum AgentsCommand {
    ...
    Run {
        ...
        /// weights — see WOVYR_MISTRALRS_GGUF_REPO/_GGUF_FILE/_TOK_MODEL_ID).
...
async fn main() -> ExitCode {
    // Structured, leveled logging to stderr (WOVYR_LOG / WOVYR_LOG_FORMAT=json), plus
    ...
    let _telemetry = wovyr_telemetry::init_logging();
...
async fn run(cli: Cli) -> wovyr_common::Result<()> {
    match cli.command {
        ...
        Command::Plugin { command } => match command {
            ...
            PluginCommand::Search {
                ...
                category,
                capability,
            } => plugin::search_cmd(query, category, capability).await,
            PluginCommand::Get {
                id,
                version,
                grants,
            } => plugin::market_install_cmd(&id, version, grants).await,
            PluginCommand::Report {
                id,
                reason,
                reporter,
            } => plugin::report_abuse_cmd(&id, &reason, reporter).await,
            PluginCommand::Reports { id } => plugin::list_abuse_reports_cmd(&id).await,
            PluginCommand::ResolveAbuse {
                id,
                report_id,
                delist,
                moderator,
            } => plugin::resolve_abuse_cmd(&id, report_id, delist, moderator).await,
            PluginCommand::DismissAbuse {
                id,
                report_id,
                reason,
                moderator,
            } => plugin::dismiss_abuse_cmd(&id, report_id, &reason, moderator).await,
        },
...
async fn dev_cmd(addr: &str) -> wovyr_common::Result<()> {
    let socket = addr
        .parse()
        .map_err(|e| wovyr_common::Error::config(format!("invalid --addr `{addr}`: {e}")))?;
    println!("Starting Wovyr dev server on http://{addr} (Ctrl-C to stop)");
    wovyr_server::serve(socket).await
}

fn login_cmd(server: String, token: Option<String>) -> wovyr_common::Result<()> {
    let token = token
        .or_else(|| std::env::var("WOVYR_TOKEN").ok())
        .filter(|t| !t.is_empty())
        .ok_or_else(|| {
            wovyr_common::Error::config("no token provided; pass --token or set WOVYR_TOKEN")
        })?;
...
fn logout_cmd() -> wovyr_common::Result<()> {
...
fn whoami_cmd() -> wovyr_common::Result<()> {
    match config::load_credentials()? {
        Some(creds) => println!("{} (token {})", creds.server, creds.masked_token()),
        None => println!("Not logged in. Run `wovyr login --server <url> --token <token>`."),
    }
    Ok(())
}
...
async fn build_local_gateway(provider: &str) -> wovyr_common::Result<Gateway> {
    match provider {
        "auto" => Ok(Gateway::from_env()),
        "anthropic" => {
            let backend = wovyr_provider::AnthropicProvider::from_env()?;
            Ok(Gateway::new(Box::new(backend)))
        }
        "mistralrs" => {
            #[cfg(feature = "mistralrs")]
            {
                let backend = wovyr_provider::MistralRsProvider::from_env().await?;
                Ok(Gateway::new(Box::new(backend)))
            }
            #[cfg(not(feature = "mistralrs"))]
            {
                Err(wovyr_common::Error::config(
                    "provider \"mistralrs\" needs a build with --features mistralrs (this \
                     binary was built without it)",
                ))
            }
        }
        other => Err(wovyr_common::Error::config(format!(
...
async fn run_local(
    ...
    if interactive_ui {
        registry.register(std::sync::Arc::new(
            wovyr_tools::UiPresentTool::new(std::sync::Arc::new(ui_present::StdinUiPresenter))
                .unrestricted(),
        ));
    }
    ...
    if !def.spec.mcp_servers.is_empty() {
        ...
        let cache = wovyr_tools::McpClientCache::default();
        let ids = cache
            ...
            .await
            .map_err(|e| {
                wovyr_common::Error::config(format!("resolving agent spec.mcp_servers: {e}"))
            })?;
...
async fn run_remote(
    ...
    let base = server
        ...
        .filter(|s| !s.is_empty())
        .ok_or_else(|| {
            wovyr_common::Error::config(
                "no server configured; pass --server <url>, run `wovyr login`, or use --local",
            )
        })?;
    let base = base.trim_end_matches('/');

    let manifest = std::fs::read_to_string(file).map_err(|e| {
        wovyr_common::Error::config(format!("could not read agent file {file}: {e}"))
    })?;
    ...
    let resp = request
        .send()
        .await
        .map_err(|e| wovyr_common::Error::provider(format!("request to {url} failed: {e}")))?;

    let status = resp.status();
    let payload: Value = resp
        .json()
        .await
        .map_err(|e| wovyr_common::Error::provider(format!("decoding response failed: {e}")))?;
    ...
    if !status.is_success() {
        ...
        return Err(wovyr_common::Error::provider(format!(
