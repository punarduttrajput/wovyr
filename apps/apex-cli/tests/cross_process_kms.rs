...
//! live" manual check (CLAUDE.md's apex-server bullet: "a CLI-sealed memory
//! record was read back through a separately-running server") that cross-process
//! CLI↔server KMS sharing actually works.
//!
//! Spawns the real `apex` binary twice against a shared scratch `HOME` (so both
//! processes resolve the identical `~/.apex/kms` root key + `~/.apex/memory`
...
//! freshly generate-on-first-use KMS root key), once as `apex dev` (the embedded
...
//! `apex` binary (via `CARGO_BIN_EXE_apex`, Cargo's standard mechanism for a
...
/// concurrent runs (and the developer's own real `~/.apex`) never collide.
fn scratch_home() -> std::path::PathBuf {
    static COUNTER: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
    let dir = std::env::temp_dir().join(format!(
        "apex_cli_cross_process_kms_{}_{}_{}",
        std::process::id(),
...
/// Kills the spawned `apex dev` server on drop, so a test failure (panic before
...
async fn cli_sealed_memory_record_is_readable_through_a_separately_started_server() {
    ...
    // generated and persisted under the scratch HOME's ~/.apex/kms.
    let put = Command::new(bin)
        ...
        .output()
        .await
        .expect("spawn `apex memory put`");
    assert!(
        put.status.success(),
        "`apex memory put --sensitive` failed: stdout={} stderr={}",
        String::from_utf8_lossy(&put.stdout),
        String::from_utf8_lossy(&put.stderr)
    );

    // 2. Start a *separate* `apex dev` server against the SAME scratch HOME, so it
    // resolves the identical root key + memory store the CLI process just wrote.
    // `APEX_ALLOW_ANONYMOUS=1` (loopback-only bind, so this is safe) +
    // `APEX_PLATFORM_ADMINS` grants the test's principal the `memory:read` scope
    ...
    let mut server = ServerGuard(
        Command::new(bin)
            ...
            .env("HOME", &home)
            .env("USERPROFILE", &home)
            .env("APEX_ALLOW_ANONYMOUS", "1")
            .env("APEX_PLATFORM_ADMINS", ADMIN_PRINCIPAL)
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .kill_on_drop(true)
            .spawn()
            .expect("spawn `apex dev`"),
    );
    ...
    assert!(
        healthy,
        "apex dev server never became healthy on 127.0.0.1:{port}"
    );
    ...
    let resp: serde_json::Value = client
        .get(&records_url)
        .header("X-Apex-Tenant", "default")
        .header("X-Apex-Principal", ADMIN_PRINCIPAL)
        .send()
        .await
