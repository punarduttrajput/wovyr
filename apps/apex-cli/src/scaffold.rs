//! `apex plugin new` / `apex plugin build` — the plugin authoring scaffold
//! (RM-AIM-P3 ECO-302).
//!
//! `new` generates a buildable plugin project: a `wasm32-wasip1` bin crate
//! using `apex-plugin-sdk`'s typed entry point, plus a `plugin.yaml` whose
...
//! existing supply chain applies unchanged: `apex plugin sign` → `trust` →
//! `install`.

use apex_common::{Error, Result};
use apex_plugin::{Artifact, PluginManifest};
use std::path::{Path, PathBuf};

/// `apex plugin new <name>` — generate a plugin project under `<dir>/<name>`.
///
/// `sdk_path` points the generated project at a local `apex-plugin-sdk`
...
[dependencies]
# Until apex-plugin-sdk is published to crates.io, generate this project with
# `apex plugin new --sdk-path <apex-repo>/crates/apex-plugin-sdk` so the
# dependency resolves locally.
apex-plugin-sdk = {sdk_dep}
...
"#,
    ...
    std::fs::write(
        ...
        format!(
            r#"apiVersion: plugin.apex.io/v1
kind: Plugin
metadata:
  name: {name}
  version: 0.1.0
...
capabilities:
  - kind: tool
    ...
    entry: {name}.wasm
    sandbox: wasm
# Computed by `apex plugin build` into dist/plugin.yaml — never hand-edit digests.
artifacts: []
"#
        ),
    )?;
...
An Apex plugin (a `wasm32-wasip1` tool capability). Edit `src/main.rs` and
`plugin.yaml`, then:

```bash
apex plugin build {name}                       # compile + stage dist/ with computed digests
apex plugin keygen {publisher}                 # once: a publisher signing keypair
apex plugin sign --key {publisher}.key --manifest {name}/dist/plugin.yaml
apex plugin trust {publisher} --key {publisher}.pub   # once, on the installing node
apex plugin install {name}/dist
apex plugin enable {publisher}/{name}
apex plugin run {name}.run --input '{{"name": "Apex"}}'
```
"#
        ),
    )?;
    ...
    println!("Next: `apex plugin build {}`", project.display());
    Ok(())
}

/// `apex plugin build <project>` — compile the project to `wasm32-wasip1` and
...
/// computed `sha256:` digest, ready for `apex plugin sign` + `install`.
pub fn build_cmd(project: &str, out: Option<&str>) -> Result<()> {
    ...
    println!(
        "Next: `apex plugin sign --key <publisher>.key --manifest {}` then `apex plugin install {}`",
...
fn wasm_entry(manifest: &PluginManifest) -> Result<String> {
    ...
    match entries.as_slice() {
        ...
        many => Err(Error::invalid(format!(
            "plugin.yaml declares {} distinct wasm entries ({}) — `apex plugin build` \
...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("apex_scaffold_{tag}_{}", std::process::id()));
...
"#,
    ...
    /// The ECO-302 acceptance round trip: `apex plugin new` → `apex plugin
    /// build` (real `cargo build --target wasm32-wasip1`) → sign → verified
    /// install — **no hand-edited digests anywhere**. Runs the exact
    /// verify/stage/register core `apex plugin install` runs, against scratch
    /// directories (never the real `~/.apex`). Skips cleanly when the wasm
    ...
    fn scaffolded_project_builds_signs_and_installs_with_no_hand_edited_digests() {
        ...
        let mut trust = apex_plugin::TrustStore::new();
        trust.trust("acme", std::fs::read(root.join("acme.pub")).unwrap());
        let mut engine = apex_plugin::PluginEngine::new(crate::plugin::platform_api(), trust)
        ...
        #[cfg(feature = "plugin-wasi")]
        {
            ...
            let mut engine = engine.with_runtime(std::sync::Arc::new(
                apex_plugin::WasiCapabilityRuntime::new().unwrap(),
            ));
            ...
            let response = tokio::runtime::Runtime::new()
                ...
                .block_on(registry.execute(
                    "greeter.run",
                    &ToolContext::default(),
                    ToolRequest::new(serde_json::json!({ "name": "Apex" })),
                ))
                .unwrap();
            assert_eq!(
                response.payload,
                serde_json::json!({ "greeting": "Hello, Apex!" })
            );
        }
