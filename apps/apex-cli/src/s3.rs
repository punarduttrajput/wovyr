//! A minimal S3-compatible object-storage client for `apex admin backup|restore`'s
//! remote destination (GA-002 §4.1, [A2-reliability-ha-dr.md](../../../docs/18-roadmap/v1.0/A2-reliability-ha-dr.md)) —
//! `apex admin backup <dest>` only ever wrote to a local filesystem path before this,
...
//! crate/version `apex-events` already uses for webhook signing) rather than the
...
//! and `sha2` are already unconditional `apex-cli` dependencies, and `hmac` is
...
/// variables so `apex admin backup s3://bucket/prefix` needs no CLI flags beyond
...
impl S3Config {
    /// `APEX_S3_ENDPOINT`, `APEX_S3_REGION` (default `us-east-1`),
    /// `APEX_S3_ACCESS_KEY_ID`, `APEX_S3_SECRET_ACCESS_KEY`.
    pub fn from_env() -> Result<Self> {
        Ok(Self {
            endpoint: require_env("APEX_S3_ENDPOINT")?,
            region: std::env::var("APEX_S3_REGION").unwrap_or_else(|_| "us-east-1".to_string()),
            access_key_id: require_env("APEX_S3_ACCESS_KEY_ID")?,
            secret_access_key: require_env("APEX_S3_SECRET_ACCESS_KEY")?,
        })
    }
}
...
/// A minimal S3-compatible client: enough for `apex admin backup|restore` to
...
impl S3Client {
    ...
    fn host(&self) -> Result<String> {
        let url = reqwest::Url::parse(&self.config.endpoint).map_err(|e| {
            Error::config(format!(
                "invalid APEX_S3_ENDPOINT `{}`: {e}",
                self.config.endpoint
            ))
        })?;
        let host = url.host_str().ok_or_else(|| {
            Error::config(format!(
                "APEX_S3_ENDPOINT `{}` has no host",
                self.config.endpoint
            ))
        })?;
        Ok(match url.port() {
...
/// dependency-free approach `apex-server`'s `hardening.rs` uses for HTTP-date
...
/// same relative-path structure `apex admin backup`'s local manifest already
