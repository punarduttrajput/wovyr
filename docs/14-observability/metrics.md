...
# examples
wovyr_api_requests_total{route,status}
wovyr_api_request_duration_seconds{route}        (histogram)
wovyr_workflow_executions_total{status}
wovyr_llm_tokens_total{model,type}
wovyr_llm_cost_usd_total{tenant,model}
wovyr_tool_queue_seconds{pool}                    (histogram)
wovyr_memory_retrieval_seconds{tier}             (histogram)
```
...
```text
wovyr_llm_cost_usd_total{tenant,project,model}
wovyr_tool_executions_total{tenant,tool,status}
wovyr_memory_records{namespace,tier}
