...
Regenerate: cargo build -p apex-cli && python3 scripts/gen-cli-docs.py
...
**Version:** generated from `apex 0.3.0`  
...
Commands:
  ...
  auth       Manage API keys for `APEX_AUTH_MODE=apikey` (RM-GA-P1 SEC-101)
  admin      Backup and restore the local `~/.apex` state directory (RM-GA-P2 DR-1001)
...
Usage: apex login [OPTIONS] --server <SERVER>

Options:
      --server <SERVER>  Server base URL (e.g. https://api.apex.example.com)
      --token <TOKEN>    Access token. Falls back to the APEX_TOKEN environment variable
  -h, --help             Print help
```
...
Usage: apex agents run [OPTIONS] --file <FILE>
...
## `apex workflows validate`

```text
Compile-check a workflow definition

Usage: apex workflows validate --file <FILE>
...
Usage: apex workflows run [OPTIONS] --file <FILE>
...
## `apex workflows approve`

```text
Approve a suspended human task and resume the execution

Usage: apex workflows approve [OPTIONS] --file <FILE> --id <ID> --task <TASK>
...
## `apex workflows signal`

```text
Deliver a timer/event signal to a waiting execution and resume it

Usage: apex workflows signal [OPTIONS] --file <FILE> --id <ID>
...
Usage: apex workflows list [OPTIONS]
...
Usage: apex workflows show --id <ID>
...
Usage: apex workflows tick --file <FILE>
...
## `apex workflows schedule`

```text
Manage recurring schedules

Usage: apex workflows schedule <COMMAND>
...
## `apex workflows schedule create`

```text
Register a recurring schedule that starts a workflow on an interval

Usage: apex workflows schedule create [OPTIONS] --file <FILE> --id <ID>
...
## `apex workflows schedule list`

```text
List registered schedules

Usage: apex workflows schedule list
...
Usage: apex memory put [OPTIONS] --content <CONTENT>
...
Usage: apex memory query [OPTIONS] <QUERY>
...
Usage: apex memory compact [OPTIONS]
...
Usage: apex plugin <COMMAND>

Commands:
  new            Scaffold a new plugin project: a buildable wasm tool + manifest (digests are computed by `apex plugin build`, never hand-edited)
  ...
  keyless-init   Set up keyless trust on this node: a dev CA + the pinned trust config (`~/.apex/plugins/keyless.json`, ADR-0009)
  ...
  pack           Bundle a package directory into a single distributable `.apexpkg` file
...
```text
Scaffold a new plugin project: a buildable wasm tool + manifest (digests are computed by `apex plugin build`, never hand-edited)

Usage: apex plugin new [OPTIONS] <NAME>
...
Options:
      ...
      --sdk-path <SDK_PATH>    Path to a local apex-plugin-sdk checkout, emitted as a `path` dependency (use `<apex-repo>/crates/apex-plugin-sdk` until the SDK is published to crates.io)
  -h, --help                   Print help
```
...
Usage: apex plugin build [OPTIONS] [PROJECT]

Arguments:
  [PROJECT]  Plugin project directory (from `apex plugin new`) [default: .]
...
Usage: apex plugin keygen [OPTIONS] <PUBLISHER>
...
Usage: apex plugin sign [OPTIONS] --key <KEY> --manifest <MANIFEST>
...
## `apex plugin keyless-init`

```text
Set up keyless trust on this node: a dev CA + the pinned trust config (`~/.apex/plugins/keyless.json`, ADR-0009)

Usage: apex plugin keyless-init [OPTIONS]
...
Options:
      ...
      --ca-key <CA_KEY>      CA key path (defaults to `~/.apex/plugins/keyless-ca.key`)
  -h, --help                 Print help
```
...
Usage: apex plugin pack [OPTIONS] <DIR>

Arguments:
  <DIR>  Path to the plugin package directory

Options:
      --out <OUT>  Output file (defaults to `<name>-<version>.apexpkg`)
  -h, --help       Print help
```
...
Usage: apex plugin trust --key <KEY> <PUBLISHER>
...
Usage: apex plugin install [OPTIONS] <DIR>
...
Usage: apex plugin upgrade [OPTIONS] <DIR>
...
## `apex plugin rollback`

```text
Roll a plugin back to its previous version

Usage: apex plugin rollback <ID>
...
Usage: apex plugin run [OPTIONS] <CAPABILITY>
...
## `apex plugin uninstall`

```text
Uninstall a plugin and remove its staged artifacts

Usage: apex plugin uninstall <ID>
...
Usage: apex plugin publish [OPTIONS] <SOURCE>

Arguments:
  <SOURCE>  Package directory (required with `--key`) or a pre-signed `.apexpkg` file
...
Usage: apex plugin search [OPTIONS] [QUERY]
...
Usage: apex plugin get [OPTIONS] <ID>
...
Usage: apex plugin report [OPTIONS] <ID> <REASON>
...
## `apex plugin resolve-abuse`

```text
Resolve an open abuse report as valid, optionally delisting the listing

Usage: apex plugin resolve-abuse [OPTIONS] <ID> <REPORT_ID>
...
## `apex plugin dismiss-abuse`

```text
Dismiss an open abuse report as not actionable

Usage: apex plugin dismiss-abuse [OPTIONS] <ID> <REPORT_ID> <REASON>
...
Usage: apex kms rotate --tenant <TENANT>
...
Usage: apex kms destroy [OPTIONS] --tenant <TENANT>
...
Usage: apex auth rotate [OPTIONS] <KEY_ID>
...
Usage: apex admin <COMMAND>

Commands:
  backup   Snapshot `~/.apex` into `<dest>` (agents, secrets, memory, workflows, tenancy, kms, and every other local store), quiescing every DUR-403-locked store directory for a consistent point-in-time copy
  restore  Restore `~/.apex` from a backup made by `apex admin backup`. Overwrites the live `~/.apex` — irreversible for anything written there since the backup was taken
...
```text
Snapshot `~/.apex` into `<dest>` (agents, secrets, memory, workflows, tenancy, kms, and every other local store), quiescing every DUR-403-locked store directory for a consistent point-in-time copy

Usage: apex admin backup <DEST>

Arguments:
  <DEST>  Destination directory to write the backup into (created if missing), or an `s3://bucket/prefix` URI for a remote object-storage destination (needs `APEX_S3_ENDPOINT`/`APEX_S3_ACCESS_KEY_ID`/`APEX_S3_SECRET_ACCESS_KEY`, and optionally `APEX_S3_REGION`)
...
```text
Restore `~/.apex` from a backup made by `apex admin backup`. Overwrites the live `~/.apex` — irreversible for anything written there since the backup was taken

Usage: apex admin restore [OPTIONS] <SRC>

Arguments:
  <SRC>  Source backup directory, as produced by `apex admin backup`, or an `s3://bucket/prefix` URI matching a remote backup's destination
...
Usage: apex admin migrate --target <TARGET> --database-url <DATABASE_URL>
