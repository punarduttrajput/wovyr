...
Scope: **the single-node appliance** (one `wovyr` binary, local `~/.wovyr`
...
Every durable store the CLI/server share lives under `~/.wovyr`: agents,
...
- **The KMS root key**, when sourced via the recommended
  `WOVYR_KMS_ROOT_KEY` production mode — by design, it is never written to
  disk in that mode, so there is nothing under `~/.wovyr` for a directory
...
- **A Postgres-backed marketplace registry** (`PostgresRegistryStore`, the
  ...
  `~/.wovyr/marketplace`; back it up with `pg_dump` on your normal Postgres
  DR schedule.
...
- `backup` acquires the cross-process advisory lock
  ...
  every existing store directory under `~/.wovyr` for the duration of the
...
- `restore` verifies every entry's digest against the manifest **before**
  touching the live `~/.wovyr`, so a corrupt or truncated backup fails closed
...
- `restore` requires `--yes` — it overwrites the live `~/.wovyr`
...
**Mechanics** (`apps/wovyr-cli/src/s3.rs`): the local backup/restore logic
...
> host has the `wovyr` binary installed and the backup is reachable.
...
> `wovyr admin backup` on a 15-minute cadence (e.g. a `cron`/systemd timer).
...
(Prune old snapshots on your own retention policy — `wovyr admin backup`
...
1. Populate a scratch `~/.wovyr` with representative content across every
...
- [`12-deployment/docker-compose.md`](docker-compose.md) §10 — running `wovyr admin backup`/`restore` against the compose stack
