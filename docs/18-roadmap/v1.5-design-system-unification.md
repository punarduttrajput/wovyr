...
AA. **Found but explicitly out of scope**, flagged as a separate follow-up
(`task_7acf517c`) rather than folded in here since it's a distinct root cause:
two small badges (`.ava` avatar initials, `.plug .logo` in
...
and needs its own fix. **Live-toggle measurement note:** confirming the fix on
