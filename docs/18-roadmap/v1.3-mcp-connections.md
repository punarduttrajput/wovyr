...
| RM-MCX-P1-105 | MCX-105 | Connection credentials are `SecretRef`s into `wovyr-secrets`'s `Vault`; the connection store never holds a raw value |
...
client.test.ts`, run against a real `wovyr dev`), plus the negative case (an
