...
`deployment/docker-compose.yml` (one `wovyr` binary + optional Postgres +
Qdrant) and `deployment/helm/wovyr/` (the same single-binary topology,
...
security floor is `wovyr-server`'s own auth/TLS layer (RM-GA-P1). The
