...
Executions persist under `~/.wovyr/workflows`, so the human task suspends durably:
...
```bash
wovyr workflows validate -f examples/workflows/support.yaml
```

## A) High-value refund → human approval

```bash
wovyr workflows run --local -f examples/workflows/support.yaml --id big \
...
```bash
wovyr workflows approve -f examples/workflows/support.yaml --id big --task approve --decision approved
```
...
```bash
wovyr workflows run --local -f examples/workflows/support.yaml --id small \
...
```bash
wovyr workflows run --local -f examples/workflows/support.yaml --id q1 \
...
- `issueRefund` declares `compensate: reverseRefund`. If a later activity fails, the
  ...
  covered by `crates/wovyr-workflow/tests/engine.rs`.
